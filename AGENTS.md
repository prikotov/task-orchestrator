...
* **Секрет** (PEM, токен `ghs_*`/PAT/JWT/API-key, пароль) — настоящее значение хранится **только** в `.env.local`/`secrets/`. Нигде больше: ни в коде, ни в тестах, ни в коммит-сообщениях, ни в логах.
