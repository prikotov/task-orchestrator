...
namespace TaskOrchestrator\Common\Module\ChainExecution\Domain\Entity;
...
final class StaticChainExecution
{
     ...
     */
    public function checkBudgetBeforeStep(?ExecutionBudgetVo $budget, string $role): ?array
    {
        if ($budget === null) {
            return null;
        }
     ...
     * @return array{limit: float, role: string|null}|null null если бюджет в норме
     */
    public function checkBudgetAfterStep(?ExecutionBudgetVo $budget, string $role, float $stepCost): ?array
    {
        if ($budget === null) {
            return null;
        }
     ...
     */
    public function isNearTotalBudget(?ExecutionBudgetVo $budget): bool
    {
     ...
     */
    public function findRetryGroup(ExecutionStepVo $step, array $fixIterations): ?ExecutionFixIterationGroupVo
    {
