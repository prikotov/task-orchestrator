...
2. Находит файл роли в `docs/agents/roles/team/` — приоритет: `<role>.<locale>.md` под локаль AI-ролей из env `TASK_ORCHESTRATOR_LOCALE` (default `en`, например при `ru` — `<role>.ru.md`) → локаль-нейтральный `<role>.md` → любой доступный перевод `<role>.<locale>.md`. При отсутствующей настройке поиск не ограничивается default `en`: роль может быть найдена как `.en.md`, `.ru.md`, `.zh.md`, любой другой локализованный файл или файл без суффикса. Выводит его **относительный путь** (от project root).
