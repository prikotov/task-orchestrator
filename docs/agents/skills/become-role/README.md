...
2. Находит файл роли в `docs/agents/roles/team/`. Если передан путь к локализованному файлу и `TASK_ORCHESTRATOR_LOCALE` не задана или пуста, локаль выводится из суффикса (`<role>.ru.md` → `ru`). Иначе используется локаль AI-ролей из env (default `en`). Дальнейший приоритет: `<role>.<locale>.md` → локаль-нейтральный `<role>.md` → любой доступный перевод `<role>.<locale>.md`. Поэтому отсутствие настройки не мешает загрузить роль, существующую только в одной локали. Скрипт выводит **относительный путь** от project root (корня проекта).
