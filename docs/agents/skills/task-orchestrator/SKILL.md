...
| `--timeout` | `-t` | Таймаут на шаг (секунды). При явном указании переопределяет `chain.timeout` | из `chain.timeout`, иначе `600` (static/dynamic) |
...
| `--max-time` | Макс. время сессии в секундах. При явном указании переопределяет `chain.max_time` | из `chain.max_time`, иначе `3600` |
