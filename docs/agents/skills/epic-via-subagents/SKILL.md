...
**Подготовка PR — перевод задачи в `done` до запроса ревью.** Убедись, что сабагент запушил код и перевёл PR в ready, затем переведи задачу в `done` по правилам проекта — CLI `todo-md` проставит статус, перенесёт файл и обновит ссылки; отметь задачу в плане эпика — и запушь. После апрувов ревьюверов в PR-ветку не пушится ничего (канон: «Подготовка PR», [`pull-request.md`](../../../../docs/git-workflow/pull-request.md)).
...
- [ ] Подготовка PR выполнена в Шаге 6: задача переведена в `done` до запроса ревью.
