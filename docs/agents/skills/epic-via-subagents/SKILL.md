...
Если задача до начала работы требует изучить большой объём внешних материалов (порядка 5–10 тысяч строк кода или документации), табличного soft-таймаута не хватает: для таких задач C3+ стартовый soft — не менее 1800 сек. После перезапуска по таймауту увеличивай soft (примерно ×1.5), а не повторяй прежнее значение.
