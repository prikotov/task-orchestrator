...
**Прецеденты для переиспользования:** <пути к похожим use case, фикстурам, паттернам контроллеров; укажи «нет», если задача пионерная>
...
3. Сохрани прогресс черновым коммитом в формате Conventional Commits: `git add -A && git commit -m "chore(<scope>): wip <что сделано> / черновик <что сделано>" && git push`, где `<что сделано>` — короткое описание завершённого этапа (по правилам обычного subject). Типа `WIP` нет в допустимых, ссылки вида «PR #N:» в subject/теле не вставляй (парсируются как футер) — формат черновых коммитов закреплён в git-workflow проекта-потребителя.
4. Перезапусти с уточняющим промптом. Увеличь soft-таймаут относительно предыдущего запуска (например, ×1.5), а не повторяй прежнее значение.
...
4. Если все ревьюверы дали апрув → переходи к **Шагу 7**.
...
2. Смержи PR со сквошем (`gh pr merge <N> --squash`), чтобы промежуточные и черновые коммиты не попадали в историю ветки эпика.
3. Удали подветку задачи.
4. Переходи к следующей задаче (**Шаг 1**).
...
3. Создай финальный PR из ветки эпика в `master`. Перед слиянием получи подтверждение пользователя (порядок — в правилах PR проекта-потребителя); мерж выполняй со сквошем, чтобы история основной ветки осталась без черновых коммитов.
...
### Поправка на эталонные задачи

Если задача читает внешний эталон большого объёма (5–10k строк — например, исходники Yii2 при переносе функциональности), чтение эталона съедает заметную часть бюджета запуска, и базовых значений не хватает — подтверждено перезапусками на трёх ретро (2026-09-14, 2026-09-16, 2026-09-23).

- Для таких задач C3+ стартовый soft — не менее 1800 сек.
- После перезапуска по таймауту увеличивай soft (примерно ×1.5), а не повторяй прежнее значение.
