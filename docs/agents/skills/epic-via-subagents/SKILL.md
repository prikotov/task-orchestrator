...
**Done-sync задачи — до запроса ревью.** По канону «Подготовка PR» ([`pull-request.md`](../../../../docs/git-workflow/pull-request.md)) задача переводится в `done` и пушится до запроса ревью; после апрува в PR-ветку не пушится ничего. Доработка по замечаниям это не нарушает — она идёт до апрува.
...
3. Повтори запуск ревью для каждого ревьювера.
4. Если все ревьюверы дали апрув → переходи к **Шагу 7** (мерж).
...
- [ ] Done-sync задачи выполнен в Шаге 6 до запроса ревью — задача в `todo/done/`, план эпика отмечен.
- Если любой из шагов пропущен — **остановись** и вернись к пропущенному.

1. Смержи PR.
2. Удали подветку задачи.
3. Переходи к следующей задаче (**Шаг 1**).
