...
**Переведи задачу в `done` до запроса ревью.** Убедись, что сабагент запушил код и перевёл PR в ready, затем переведи задачу в `done` — обнови статус, отметь задачу в плане эпика, перенеси файл — и запушь в ветку PR. После апрувов ревьюверов в PR-ветку не пушится ничего.
...
- [ ] Задача переведена в `done` до запроса ревью (Шаг 6).
