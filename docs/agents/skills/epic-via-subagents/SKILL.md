...
6. Сделай push от GitHub App по [безопасному HTTPS-рецепту](../../../../docs/guide/agent-identity.md#push-pr-веток-только-токеном-бота); обычный `git push` и SSH владельца запрещены.
7. Переведи PR из draft в ready: `gh pr ready <PR_NUMBER>`.
```

4. Создай подветку от ветки эпика, закоммить файл задачи и сделай bot push по безопасному HTTPS-рецепту.
5. Создай draft PR из подветки в ветку эпика.
6. Обнови секцию инструкций в задаче: впиши реальный `[<PR_NUMBER>](<PR_LINK>)`. Закоммить и повтори bot push по безопасному HTTPS-рецепту.
...
[PR #<PR_NUMBER>](<PR_LINK>) уже существует, после правок закоммить и повтори bot push по безопасному HTTPS-рецепту.
```
...
2. Оцени прогресс: `git diff --stat`, `make check 2>&1 | tail -20`.
3. Сделай WIP-коммит: `git add -A && git commit -m "WIP: <TASK-ID>"`, затем повтори bot push по безопасному HTTPS-рецепту.
...
4. Если все ревьюверы дали апрув → переходи к **Шаг 7**.

### **Шаг 7:** Мерж PR сабагента в ветку эпика

🔴 Закрытие PR, удаление remote-ветки, force-push и массовое перемещение/удаление файлов выполняй только после отдельного явного согласия пользователя. Installation token применяй только внутри безопасного рецепта и никогда не выводи в stdout/output или журналы.
...
6. Закоммить и повтори bot push по безопасному HTTPS-рецепту.
7. Перед запросом approval по журналу собственных действий удостоверься, что все push подветки выполнены по безопасному рецепту; не заявляй о недоступной через GitHub API проверке полной истории отправителей.
8. После отдельного явного согласия пользователя смерджи PR.
9. Удали локальную подветку задачи. Удаление remote-ветки требует отдельного явного согласия пользователя.
10. Переходи к следующей задаче (**Шаг 1**).
...
2. Запусти финальные проверки: `make check`.
3. Выполни bot push по безопасному HTTPS-рецепту и создай финальный PR из ветки эпика в `master`.
4. Обнови файл эпика: `status: done`, `pr: <ссылка на финальный PR>`; закоммить метаданные и повтори bot push до запроса approval.
5. Перед запросом approval по журналу собственных действий удостоверься, что все push ветки эпика выполнены по безопасному рецепту; не заявляй о недоступной через GitHub API проверке полной истории отправителей.
6. Merge и удаление remote-ветки выполняй только после отдельного явного согласия пользователя.
7. Отчитайся пользователю. Пример отчёта:
