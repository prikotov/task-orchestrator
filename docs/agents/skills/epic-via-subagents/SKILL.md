...
3. Не коммить незавершённую работу чужой рукой: незакоммиченный diff — состояние, на котором остановился исполнитель, это и есть точка продолжения.
4. Перезапусти с уточняющим промптом: «незакоммиченные изменения — результат предыдущего запуска; изучи их через `git status` и `git diff` и продолжи с этого места». Увеличь soft-таймаут относительно предыдущего запуска (например, ×1.5), а не повторяй прежнее значение.
