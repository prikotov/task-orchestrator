...
Финальный PR: `<epic-branch>` → `main`.

⚠️ **Критически важно:** подветки задач создаются **от ветки эпика**, а не от `main`. Ветка эпика содержит весь код, который ещё не влит в `main`.
...
5. Создай ветку эпика от актуального `main`.
6. Запиши имя ветки в поле `branch` файла эпика. Закоммить и сделай push через `bin/agent-push`.
...
6. Сделай push через `bin/agent-push <owner>/<repo> <branch>`.
7. Переведи PR из draft в ready: `gh pr ready <PR_NUMBER>`.
```

4. Создай подветку от ветки эпика, закоммить файл задачи и сделай push через `bin/agent-push <owner>/<repo> <task-subbranch>`.
5. Создай draft PR из подветки в ветку эпика.
6. Обнови секцию инструкций в задаче: впиши реальный `[<PR_NUMBER>](<PR_LINK>)`. Закоммить и повторно вызови `bin/agent-push`.
...
[PR #<PR_NUMBER>](<PR_LINK>) уже существует, после правок закоммить и сделай push через `bin/agent-push`.
```
...
3. Сделай WIP-коммит: `git add -A && git commit -m "WIP: <TASK-ID>"`, затем вызови `bin/agent-push`.
...
6. Закоммить и сделай push через `bin/agent-push`.
7. Перед запросом approval сверься с журналом собственных действий и удостоверься, что все push подветки выполнялись через `bin/agent-push`; не заявляй о недоступной через GitHub API проверке полной истории отправителей.
...
3. Сделай push ветки эпика через `bin/agent-push <owner>/<repo> <epic-branch>` и создай финальный PR в `main`.
4. Обнови файл эпика: `status: done`, `pr: <ссылка на финальный PR>`; сделай отдельный metadata commit и повторный bot push до merge.
5. Merge выполняй только после отдельного явного согласия пользователя; удаление remote-ветки согласуй отдельно.
6. Отчитайся пользователю. Пример отчёта:
...
- [ ] Финальный PR из ветки эпика в `main` создан.
