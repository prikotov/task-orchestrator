...
> Этот документ — про внутренности: параметры, окружение, контроль таймаутов,
...
| `--reasoning`     | —          | Усилие рассуждения/обдумывания (pi: `--thinking`, codex: `model_reasoning_effort=…`) | профиль роли |
| `[prompt text]`   | —          | Промпт. Если не указан — читается из стандартного ввода                                     | —            |

### Резолв `--runner`/`--provider`/`--model`/`--reasoning`

Приоритет: **параметр CLI → env → профиль роли → значение по умолчанию**.
...
| `REASONING` | Усилие рассуждения/обдумывания по умолчанию |

### Наблюдаемость

| Переменная              | Назначение |
|-------------------------|------------|
| `CHAINS_CONFIG`         | Путь к `chains.yaml` (по умолчанию: `config/chains.yaml`) |
| `WATCH_LOG_DIR`         | Каталог run-логов (по умолчанию: `var/log/watch-subagent`) |
...
Запуск терминируется в трёх случаях: превышен целевой лимит времени (`soft`),
...
1. **In-loop проверки** (в теле `while IFS= read -r -t "$STALL_TIMEOUT"`):
   мягкий и жёсткий лимиты проверяются при каждом событии, stall — через `read -t`. Это
...
2. **Фоновый watcher по wall-clock** — страховка. Отдельный подпроцесс,
   **не** блокирован в `read`, опрашивает каждые `WATCH_WATCHER_INTERVAL` сек.
   ...
   - терминирует запуск по мягкому/жёсткому лимиту/признаку зависания по часам;
...
> мягкие сигналы (`TERM`/`INT` и собственные сигналы мягкого/жёсткого лимита/зависания) обрабатываются.
...
└── events/              # архивируется при неуспехе (или WATCH_KEEP_TMP=1)
    ...
    ├── last_event       # ts последнего события (состояние для наблюдателя)
...
### Порядок диагностики (при `soft_timeout` / `stall` / пустом выводе)

Не делай вывод о модели, промпте или контексте, не проверив сначала ошибки провайдера — это самая частая причина «зависания» сабагента.

1. Прочитай `reason` в `RUN SUMMARY` (см. таблицу выше).
2. Открой `events/runner.stderr` и прогрепай `events/events.ndjson` по HTTP-ошибкам провайдера: `429`, `5xx`, `overloaded`, `rate limit`, `Forbidden`, `timeout`, `Connection`.
3. Нашёл HTTP-ошибку → причина сеть/провайдер, **не** модель:
   - `429` / `overloaded` / `rate limit` → сервис перегружен; retry через паузу либо смена провайдера/модели.
   - `403 Forbidden` → блокировка по IP/региону (для OpenAI — нужен прокси, см. `AGENTS.md`, раздел «Запуск codex (прокси)»).
   - `5xx` / `timeout` / `Connection` → нестабильность сети/провайдера; retry, проверь прокси/сеть.
4. HTTP-ошибок нет, а `watcher_heartbeat` показывает растущий `stall_gap` при `agent_alive=yes` → основной цикл завис (real-pi edge case, обходит блокированный `read`).
5. Только если пп. 3–4 пусты → версия про модель/промпт/контекст (слишком длинный промпт, неадекватная модель, зацикливание).

🔴 **Запрещено** списывать «модель медленная» или «stall» до проверки п. 2 — чаще всего это `429`/overload провайдера.
