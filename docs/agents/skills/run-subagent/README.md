...
| `--stall-timeout` | `-t`       | Нет событий N секунд → агент завис → завершить принудительно | 360 для всех раннеров |
...
Stall-порог по умолчанию — 360 секунд для всех раннеров; PHP-раннер использует
общий idle-порог `AGENT_RUNNER_IDLE_TIMEOUT_SEC=330`. Модели любых провайдеров
могут молчать до ~5 минут в фазе ожидания или восстановления, поэтому меньшие
общие значения преждевременно срывают штатные автоповторы. Пользователь может
задать любое значение `-t`.
