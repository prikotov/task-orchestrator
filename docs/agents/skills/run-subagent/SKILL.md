...
При `SIGKILL`/OOM/закрытии терминала `trap cleanup` скрипта не отрабатывает, и
`/tmp` копит осиротевшие TMPDIR (`events.ndjson` до сотен МБ на запуск). Для
самоочистки при старте служит startup-GC: env `WATCH_TMP_GC=0` отключает его,
`WATCH_TMP_GC_MIN_AGE_MIN=N` задаёт минимальный возраст (минут, default 360) для
удаления. Фоновый watcher при гибели родителя убирает TMPDIR сам.
