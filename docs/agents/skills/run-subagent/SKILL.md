...
> **Фоновый watcher (страховка по wall-clock).** Проверки 1–4 живут внутри
> read-цикла и исполняются, только когда из пайпа пришла строка. Если основной
> цикл завис в `read` (наблюдалось на pi 0.79.6: запуск вис до внешнего SIGKILL,
> теряя `RUN SUMMARY` и архив), отдельный фоновый watcher терминирует запуск по
> часам: сам пишет `RUN SUMMARY` (с `source=watcher`) и архивирует `events/`,
> затем убивает дерево агента и основной процесс. Интервал опроса —
> env `WATCH_WATCHER_INTERVAL` (default: 5с). Каждые N секунд watcher пишет в
> `run.log` строку `watcher_heartbeat elapsed=… stall_gap=… agent_alive=…` —
> это диагностическая улика «основной цикл завис» (растущий `stall_gap` при
> живом агенте). На нормальных запусках watcher не вмешивается (in-loop проверки
> срабатывают раньше) и останавливается в `cleanup`.
...
Env `WATCH_WATCHER_INTERVAL` — интервал опроса фонового watcher в секундах
(default: 5).
