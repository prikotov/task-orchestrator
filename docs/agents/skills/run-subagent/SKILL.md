...
Запуск гарантированно терминируется по таймауту или завершению агента; при неуспехе пишется `run.log` + архив `events/` в `var/log/watch-subagent/`. Внутренние детали реализации и env-переключатели — в шапке `scripts/watch-subagent.sh`.
