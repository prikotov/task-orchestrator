...
| **Retry с backoff** | ✅ RetryingAgentRunner | ✅ Node-level retry (default: 2, max: 5), exponential backoff, error classification (FATAL/TRANSIENT) | ✅ Паритет |
...
| **Tool control (hooks)** | ❌ Нет | ✅ Per-node SDK hooks (21 событие: PreToolUse, PostToolUse, Stop, SessionStart и др.) | 🟡 Позже |
...
Archon хранит всё в 8 таблицах БД: сессии, сообщения, workflow runs, events, env vars. Для интерактивного мультплатформенного чата это оправдано. Наш оркестратор запускается, выполняет цепочку, завершается. In-memory + JSONL audit trail достаточно.
