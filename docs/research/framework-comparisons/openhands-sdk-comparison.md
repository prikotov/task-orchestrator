...
openhands-sdk/openhands/sdk/
 ...
 subagent/
 registry.py Subagent-реестр: register_agent, file-based agents
...
| Функция | OpenHands SDK |
| --- | --- |
| **Retry с backoff** | ✅ RetryMixin (tenacity, exponential backoff, 5 attempts) |
| **Бюджетный контроль** | ⚠️ Token/cost tracking через metrics, но без явных budget лимитов |
| **Итерационные циклы (fix_iterations)** | ✅ IterativeRefinementConfig (critic-driven, max_iterations) |
| **Fallback routing** | ✅ FallbackStrategy (LLM profile fallback при transient errors) |
| **Audit Trail (JSONL)** | ✅ Event stream (file-backed event store, JSON serialization) |
| **Ролевые промпты** | ✅ Jinja2-шаблоны (system_prompt, model-specific) |
| **Multiple runners** | ✅ 100+ провайдеров через LiteLLM |
| **DDD-архитектура** | ❌ SDK с modular packages (sdk, tools, workspace, agent-server) |
| **Decorator pattern** | ❌ Composition pattern (Agent = LLM + tools + context) |
| **YAML-конфигурация** | ❌ Программная конфигурация (Python API) |
| **Stuck detection (loop detection)** | ✅ StuckDetector: 4 активных паттерна зацикливания |
| **Context condensation (summarization)** | ✅ LLMSummarizingCondenser, View snapshot |
| **Security risk assessment** | ✅ SecurityRisk (UNKNOWN/LOW/MEDIUM/HIGH), LLM-based + ensemble |
| **Confirmation policies** | ✅ AlwaysConfirm / NeverConfirm / ConfirmRisky |
| **Parallel tool execution** | ✅ ParallelToolExecutor с resource-level locking |
| **Sub-agent delegation** | ✅ SubagentRegistry, DelegateTool, file-based agents |
| **Hooks (pre/post tool use)** | ✅ HookManager: 6 lifecycle event types с matcher-фильтрацией |
| **MCP-интеграция** | ✅ MCPClient, MCPToolDefinition |
| **Skills (SKILL.md)** | ✅ Формализованные SKILL.md, discovery, validation, marketplace |
| **Plugin система** | ✅ Plugin: skills + tools + hooks + MCP из Git-репозиториев |
| **Agent Server (remote execution)** | ✅ Agent Server: FastAPI + Docker sandboxing |
| **Sandboxing (Docker/K8s)** | ✅ Docker, Kubernetes, Apptainer, API-remote workspaces |
| **Critic (quality scoring)** | ✅ CriticBase: score + feedback, iterative refinement |
| **LLM Profile Store** | ✅ LLMProfileStore: файловые JSON-профили с параметрами LLM |
| **Conversation persistence** | ✅ File-backed event store, persistence_dir |
| **Conversation forking** | ✅ Fork conversation state |
| **Tool annotations (readOnlyHint и т.д.)** | ✅ ToolAnnotations (readOnly, destructive, idempotent, openWorld) |
| **Event streaming (delta)** | ✅ StreamingDelta для real-time вывода |
...
**Оркестрационная значимость:** При итерационных циклах (fix_iterations) — если агент «зацикливается» (повторяет одни и те же ошибки), нужен механизм обнаружения и остановки. Конкретно паттерн #1 (repeating action-observation) и #2 (repeating action-error) наиболее актуальны.
...
**Оркестрационная значимость:** Для длинных цепочек (implement → review → fix → review → ...) контекст может расти. Condensation позволяет работать с сколь угодно длинными сессиями. Для будущих dynamic loops — критически важно.
...
**Оркестрационная значимость:** Двухуровневая модель (risk assessment + confirmation policy) — хорошо структурированный подход.
...
**Оркестрационная значимость:** Концептуально похоже на наши fix_iterations, но с LLM-based scoring вместо shell-команд quality gates. Комбинация critic + iterative refinement — мощный паттерн: agent делает работу → critic оценивает → если score < threshold → agent дорабатывает.
...
**Оркестрационная значимость:** У OpenHands — на уровне LLM-профилей с metrics merging. Идея файловых LLM-профилей — удобна для конфигурации разных моделей под разные задачи.
...
**Оркестрационная значимость:** Декларативные свойства инструментов позволяют автоматически принимать решения о параллелизме (`readOnly` → safe to parallelize) и уровне риска (`destructive` → требуется подтверждение). Аннотации используются `ParallelToolExecutor` и `SecurityAnalyzer` совместно.
...
**Оркестрационная значимость:** Формализованная система lifecycle-хуков с matcher-фильтрацией — позволяет встраивать проверки и уведомления на ключевых этапах агентного цикла без модификации основного кода. Пример: pre-tool hook блокирует опасные команды, post-tool hook логирует результат.
...
**Оркестрационная значимость:** Для цепочек, где несколько шагов можно выполнить параллельно (например, lint + type-check + tests). Resource locking предотвращает конфликты при параллельном доступе к FS.

---

### 3.9 🟡 Sub-agent Delegation — файловые и программные агенты (`subagent/`)

**Что у них:** Система регистрации и делегирования subagent'ов:
...
**DelegateTool** позволяет агенту делегировать задачу subagent'у через tool call.

**Оркестрационная значимость:** Для будущих multi-agent сценариев: main agent делегирует sub-tasks специализированным агентам. Файловый формат (Markdown с YAML frontmatter) для определения агентов — удобен для конфигурации без кода.
...
OpenHands SDK имеет полноценный Agent Server (FastAPI) для sandboxed-выполнения в Docker/Kubernetes/Apptainer. Sandboxing — задача CI/CD инфраструктуры, не оркестратора.

### 4.2 🟢 Browser Tool (Playwright)

OpenHands SDK включает browser-use для веб-навигации. Взаимодействие с браузером — за пределами наших use cases.

### 4.3 🟢 Conversation Forking

OpenHands поддерживает forking — ветвление conversation state для exploration. Forking добавляет complexity без clear benefit для наших задач.
...
OpenHands SDK завязан на LiteLLM для multi-provider поддержки. Нам не нужна прослойка над LLM API.
