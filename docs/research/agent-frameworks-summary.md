...
> **Статус заполнения артефактов:** все 38 / 38 исследовательских артефактов, включая bb #36, T3 Code #37 и pragmatic-orchestration #38, подготовлены и включены в сводку и расчёты. Процессная приёмка pragmatic-orchestration остаётся отдельным шагом задачи; готовность исследовательского артефакта не означает закрытие evergreen-эпика.
...
> Анализ выполнен на основе 38 / 38 подготовленных исследовательских артефактов; T3 Code #37 и pragmatic-orchestration #38 входят в расчёты. TASK #38 остаётся `in_progress` до процессной приёмки. Тренды сгруппированы по значимости для архитектуры task-orchestrator.
...
**19 из 38 подготовленных исследовательских артефактов** описывают базовую или first-class модель `LLM → tool call → observation → LLM → ...`. В numerator (числитель) входят: Crush (#1), pi_agent_rust (#2), CrewAI (#3), LangGraph (#4; agent loop может быть выражен как graph cycle/superstep), AutoGen (#5), OpenHands SDK (#6), MetaGPT (#8), OpenClaw (#9), Mastra AI (#10), Claude Code (#11), Copilot Cloud Agent (#12), Docker Agent + Codex (#13), Agno (#14), Hermes Agent (#18), Kilo Code (#21), OpenCode (#22), OmO (#23), Zeroclaw (#26), **Odysseus (#27)**. После добавления qm (#32), omnigent (#33), Herdr (#34), ALK (#35), bb (#36), T3 Code (#37) и pragmatic-orchestration (#38) denominator (знаменатель) достиг `38`; ни один из семи platform/harness-слоёв не увеличивает numerator: qm и omnigent оркестрируют внешние coding-агенты через harness-контракты, Herdr запускает внешние agent loops (циклы агентов) в реальных терминалах, ALK управляет lifecycle/proof вокруг внешних host CLIs, а bb запускает их через provider plugins и опциональные workflows, не реализуя собственный LLM/tool loop. Ранее добавленные **bx-dev (#31)**, **Orca ADE (#30)** и **SwarmForge (#29)** также не увеличивали numerator: это соответственно manual workflow harness, ADE/manual orchestration layer и swarm-orchestration layer.
...
**Все 38 подготовленных исследовательских артефактов** чётко делятся на восемнадцать уровней:

| Уровень | Проекты | Что делают | Аналог в task-orchestrator |
|---|---|---|---|
| **SDK / Agent runtime** | Crush, pi_agent_rust, OpenHands SDK, Mastra AI, Claude Code, Codex, OpenClaw, Agno, Hermes Agent, Kilo Code, **OpenCode**, **OmO**, **Zeroclaw**, **Odysseus** | Работают на уровне прямых LLM API | Runner'ы (pi, codex) |
...
**29 из 38 подготовленных исследовательских артефактов** соответствуют этому критерию. Ровно учтённые проекты:
1. Crush (#1)
2. pi_agent_rust (#2)
3. CrewAI (#3)
4. OpenHands SDK (#6)
...
**14 из 38 подготовленных исследовательских артефактов** подтверждают auto-compaction / auto-summarization при context overflow:
- Crush, pi_agent_rust, OpenHands SDK, Mastra AI, Claude Code, Codex, Agno, Hermes Agent, Kilo Code, **OpenCode** (7-секционный structured template + pruning + preserve recent turns + replay), **OmO** (proactive compaction: monitor + compact **до** ошибки + dynamic context pruning: dedup/supersede/purge), **Zeroclaw** (ContextCompressor: LLM-суммаризация + HistoryPruner: orphaned tool messages + collapsed pairs + protected indices + 1.2x token safety margin), **qm** (`context-compaction.ts`), **omnigent** (`runtime/compaction.py`)
