...
> **Эпик:** [EPIC-research-agent-frameworks-comparison](../../todo/EPIC-research-agent-frameworks-comparison.todo.md)
...
> **Статус заполнения артефактов:** все 39 / 39 исследовательских артефактов, включая bb #36, T3 Code #37, pragmatic-orchestration #38 и Strands Harness SDK #39, подготовлены и включены в сводку и расчёты. Готовность исследовательского артефакта не означает закрытие evergreen-эпика.
...
| 32 | qm (yc-software) | TypeScript (Node) | `multiplayer/multi-tenant agent platform` | `per-turn scope-resolved routing` одного turn'а в один из внешних harness'ов (Pi/OpenCode/Codex/Claude Code за одним интерфейсом) поверх multi-tenant платформы; background crons/watches | `persistent` (Postgres: sessions · memory · queue · `deployment_layer` · audit) + per-scope workspace/keychain + durable sandbox (immutable digest-pinned rootfs) | platform-level: audit, scope isolation, security screening fail-closed, command policy (hard denials всегда) — **нет universal runner-/chain-level retry/backoff/CB/budget/fix_iterations** (loop делегирован harness'ам); bounded backoff только в узких местах (throttled screening) | `Harness` interface + `harness-router` (approved harness+модель per scope) + sandbox substrates за интерфейсами + skill packs из git + deployment-directory контракт (`qm` CLI, `@yc-software/qm/contract`) + plugin surfaces (Slack/web/admin/portal) | 🔴 не dependency, 🟡 заимствовать multi-harness/security-posture/skills-governance patterns | [qm-comparison.md](framework-comparisons/qm-comparison.md) ✅ |
| 33 | omnigent | Python 3.12+ | `meta-harness / multi-device collaboration platform` (мета-харнес и платформа совместной работы) | `harness abstraction` (единый контракт над 12+ харнесами; orchestrator-мозг делегирует внешним агентам; multi-agent supervision через под-агентов) | `server-managed sessions + git worktrees + managed hosts` (cloud sandboxes) | `LLM/tool-call retry+backoff + error classification` (ContextWindow/Permanent/Retryable) на **микроуровне** внутри agent loop; нет chain-level CB/gates/budget/fix_iterations | `custom YAML agents + 12+ harness backends (native tmux/PTY + SDK) + policies (function/CEL; safety/cost/routing) + OS sandbox (bwrap/seatbelt/Job Objects) + 11 cloud providers + MCP + .claude/skills/AGENTS.md + collaboration` | 🔴 не dependency, 🟡 заимствовать meta-harness/policies/sandboxing/supervision patterns | [omnigent-comparison.md](framework-comparisons/omnigent-comparison.md) ✅ |
...
| 35 | Agent Lifecycle Kit | Python 3.11–3.14 | `provider-neutral lifecycle controller / evidence layer` (нейтральный контроллер жизненного цикла и слой доказательств) | фиксированный lifecycle (жизненный цикл): intake (вход) → plan (план) → freeze (заморозка) → host-owned execution (исполнение внешним хостом) → audit (аудит) → final proof (финальное доказательство); read-only managed next action (управляемое следующее действие без записи) + Review Mesh | `file-backed` (файловые) plan manifests/locks, workflow state, runner state, adapter sessions, receipts and event logs (манифесты/замки плана, состояния, сессии, доказательства и журналы) | fail-closed checks (проверки с отказом по умолчанию) lineage/schema/gate + bounded attempts/reroutes/splits/token caps (ограниченные попытки/маршруты/разделения/токены) + host process timeouts + model-route escalation; нет chain-level CB/fix_iterations runner parity | 12 adapter descriptors/capability manifests (дескрипторов адаптеров/манифестов возможностей) + 7 `SKILL.md` + plugin manifests + public JSON schemas + host-local profiles + import mappers | 🔴 не dependency, 🟡 заимствовать frozen-plan/proof/adapter-support patterns (паттерны замороженного плана, доказательств и уровней поддержки адаптеров) | [agent-lifecycle-kit-comparison.md](framework-comparisons/agent-lifecycle-kit-comparison.md) ✅ |
| 36 | bb | TypeScript (Node.js/Electron) | `agentic IDE / control surface + optional workflow engine` (агентная среда разработки и управляющая поверхность с опциональным движком процессов) | provider-neutral threads (нейтральные потоки поставщиков), parallel external agents (параллельные внешние агенты), live steering (живое направление); отключённый по умолчанию QuickJS Workflows с `agent`/`parallel`/`pipeline`/одним уровнем nesting (вложения) | `persistent` (SQLite server + plugin DB) + workspaces/worktrees; Workflows сохраняет calls (вызовы) и воспроизводит неизменённый успешный префикс | Workflows: 2 повтора временных provider failures (ошибок поставщика) с bounded backoff (ограниченной задержкой) + 2 corrective retries (корректирующие попытки) для schema output; provider-reset plugin узкий и in-memory; нет chain-level CB/gates/cost budget/fix_iterations parity | public provider/plugin SDK + server/app/host/CLI plugins + `SKILL.md` + UI/CLI/HTTP/WebSocket/TypeScript SDK + managed worktrees | 🔴 не core dependency, 🟡 заимствовать provider/thread/durable-replay patterns, 🟢 optional manual environment (опциональная ручная среда) | [bb-comparison.md](framework-comparisons/bb-comparison.md) ✅ |
| 37 | T3 Code (pingdotgg) | TypeScript (Node.js) | `GUI-orchestrator` | нормализованный provider contract (ProviderAdapter) + durable thread поверх заменяемой сессии + decider/reactor (фиксация намерения до I/O) | server-owned event journal + command receipts (идемпотентность) + projections | capability checks до эффекта + checkpoint в скрытых Git refs + отклонение несовместимых операций ДО изменения файлов + idempotent commands | 6 harness-адаптеров (Claude Code/Codex/Cursor/Grok/OpenCode/Antigravity) + permission modes + multi-account per provider + remote access (RPC, mobile) | 🟡 заимствовать отдельные паттерны (apply: capability-контракт, BYO subscription; study: worktree, durable threads, remote) | [t3-code-comparison.md](framework-comparisons/t3-code-comparison.md) ✅ |
| 38 | pragmatic-orchestration | Python 3.11+ + Bash | `orchestration skill suite / delegation layer` | Agent Skills + `porch`: read-only review fan-out и один steerable delegate над внешними CLI-агентами; не chain engine | файловый per-run registry + mailbox с `flock`/монотонным `seq` + raw/normalized/final artifacts; sessions читает нативные stores без индекса | fail-closed event/mode contracts + явные partial/timeout/cancel/recovery exits; steer evidence ranks; нет chain-level retry/CB/gates/budget | Codex/Claude/OpenCode/Grok/Devin adapters, Gemini review-only + Agent Skills + backend capability matrix + remote-agents skill | 🔴 не dependency, 🟡 заимствовать steer/event/policy/delegation/testing patterns | [pragmatic-orchestration-comparison.md](framework-comparisons/pragmatic-orchestration-comparison.md) ✅ |
| 39 | Strands Harness SDK | Python 3.10+ + TypeScript | `SDK / agent framework with assembled harness` | встроенный `model → tool → model` loop + готовый `create_harness()`; agent-as-tool и A2A; Graph и Swarm с состоянием и ограничителями (A2A внутри Swarm пока не поддерживается) | sessions + agent state + memory + context managers (sliding window/summary/offload/pin) | типизированные stop reasons + лимиты/отмена + throttling retry с exponential backoff; tool errors возвращаются модели; нет подтверждённого universal CB | models/providers + tools/MCP + Agent Skills (`SKILL.md`) + hooks/plugins/interventions + session/memory/context managers + multi-agent plugins + Evals SDK | 🔴 не dependency, 🟡 заимствовать termination/policy/defaults/state/observability/eval patterns | [strands-harness-sdk-comparison.md](framework-comparisons/strands-harness-sdk-comparison.md) ✅ |

### Легенда колонок

| Колонка | Описание | Значения |
|---|---|---|
| **Категория** | Тип фреймворка | `single-agent`, `multi-agent`, `meta-orchestration`, `cloud/SaaS`, `CLI-agent`, `ADE / manual-orchestrator`, `terminal agent runtime / control surface`, `agentic IDE / control surface + optional workflow engine` |
...
По 39 подготовленным исследовательским артефактам об AI-agent фреймворках, инструментах, оркестраторах, ADE, skill packs, терминальных средах и lifecycle controllers можно сделать **три главных вывода**. Набор исследовательских артефактов подготовлен полностью — 39 / 39; Strands Harness SDK #39 включён в расчёты.

1. **По 39 подготовленным исследовательским артефактам не найден проект с тем же полным набором возможностей, что у `task-orchestrator`:** YAML-цепочки + retry с backoff (повтор с задержкой) + circuit breaker (предохранитель отказов) + quality gates (shell-проверки качества) + бюджетный контроль + `fix_iterations` + fallback routing (маршрутизация на запасной вариант) + JSONL audit trail (JSONL-журнал аудита). **qm** (#32) — наиболее зрелая multi-harness платформа над внешними coding-агентами, но её core не даёт chain-level retry/CB/gates/budget/fix_iterations. **omnigent** (#33) расширяет meta-harness-подход custom YAML, sandboxing и policy engine, но также не заменяет chain-level resilience. **Paperclip AI** — ближайший аналог по уровню meta-orchestrator (мета-оркестратора), но работает на уровне компании/агентов, а не chain steps (шагов цепочки). **Agent Lifecycle Kit** (#35) близок по disciplined lifecycle/proof (строгому жизненному циклу и доказательствам), но не является chain runtime (средой выполнения цепочек) и не имеет runner-level circuit breaker/fix_iterations parity (паритета предохранителя отказов и циклов исправления уровня запускателя). **Zeroclaw** (#26, Rust) — развитый single-agent runtime (среда выполнения одного агента) из исследованных, но без circuit breaker, quality gates, бюджетного контроля и chain-level retry (повтора уровня цепочки). **Duet** (Aomni) показывает, что SaaS-платформы, локальные swarm-оркестраторы (роевые оркестраторы) и Codex-skill harnesses (харнесы навыков Codex) обычно закрывают только часть этого набора.
...
3. **Ближайшие аналоги** по уровню абстракции — Archon (TypeScript/Bun, chain-level оркестрация через subprocess SDK), Paperclip AI (TypeScript/Node.js, company-level мета-оркестратор), SwarmForge (#29), Orca ADE (#30), bx-dev (#31), qm (#32), omnigent (#33), Herdr (#34), bb (#36), T3 Code (#37) и pragmatic-orchestration (#38). Archon не имеет circuit breaker, quality gates или бюджетного контроля. Paperclip AI имеет развитый budget enforcement, run recovery и plugin system, но не поддерживает chains, circuit breaker или quality gates. **SwarmForge** близок по ролевой координации. **Orca ADE** близок по worktree-first ручной оркестрации. **bx-dev** близок к нашему `task-via-subagents`. **Herdr** находится уровнем ниже: это постоянный сервер реальных терминалов с агентными состояниями, событиями, рабочими деревьями и единым CLI/socket API, но без предписанного рабочего процесса. **bb** находится между ADE и workflow engine (движком рабочих процессов): core (ядро) даёт provider-neutral threads (нейтральные потоки поставщиков), а отключённый по умолчанию Workflows plugin (плагин процессов) — устойчивое QuickJS-исполнение с параллельными агентами, schema output (выводом по схеме) и воспроизведением успешных вызовов. **T3 Code** добавляет долговечный server-owned thread (принадлежащий серверу поток) и capability-first provider contract (контракт поставщика от возможностей), а **pragmatic-orchestration** — тонкий skill+CLI-слой с живым steer, закрытыми событиями и fail-closed матрицей режимов. **Agent Lifecycle Kit** находится смежным уровнем: lifecycle/evidence controller (контроллер жизненного цикла и доказательств) с frozen plans (замороженными планами), receipts (доказательствами), audits (аудитами) и adapter support levels (уровнями поддержки адаптеров), но без собственного agent loop (цикла агента) и без chain-level resilience (устойчивости уровня цепочки) `task-orchestrator`. Ни один из одиннадцати перечисленных ближайших аналогов, как и смежный ALK, не подходит как core dependency (основная зависимость): SwarmForge ценен для governance/handoff/team topology (управления, передачи работ и топологии команды), Orca — для fan-out comparison (веерного сравнения) и live worktree monitoring (живого наблюдения за рабочими деревьями), bx-dev — для session UX (пользовательского опыта сессии) и merge-governance (управления слиянием), Herdr — для lifecycle authority (полномочий жизненного цикла), atomic prompt-and-wait (атомарной отправки промпта и ожидания), ownership (владения) и worktree provenance (происхождения рабочего дерева), bb — для provider-neutral threads (нейтральных потоков поставщиков), unified surfaces (единых интерфейсов) и durable call replay (устойчивого воспроизведения вызовов), ALK — для frozen authority (замороженных полномочий), proof receipts (доказательств), adapter support taxonomy (таксономии поддержки адаптеров) и provider-neutral model-class routing (нейтральной маршрутизации классов моделей). У Herdr нет universal runner-/chain-level retry/backoff/CB (универсальных повторов/задержек/предохранителя отказов уровня запуска или цепочки), quality gates, budget или машинного результата шага, сопоставимого с `task-orchestrator`; у bb нет chain-level CB, shell quality gates, cost budget или `fix_iterations`; у ALK нет chain-level CB/fix_iterations/JSONL runner parity (паритета по предохранителю отказов, циклам исправления и JSONL-аудиту уровня цепочки), но есть подробнее описанная proof vocabulary (лексика доказательств).
...
| **Durable call ledger + unchanged-prefix replay** (устойчивый журнал вызовов и воспроизведение неизменённого префикса) | bb Workflows | Сохранять вызовы, результаты и retry budget (бюджет повторов); после перезапуска воспроизводить только неизменённый успешный префикс | Полезно для dynamic chains, но требует явной идемпотентности и доказательств побочных эффектов |
| **Frozen plan + receipt-bound gates** (замороженный план и ворота, привязанные к доказательствам) | Agent Lifecycle Kit | Перед выполнением фиксировать digest (хеш) плана, write-scope (область записи), required evidence (обязательные доказательства) and gate receipts (доказательства ворот) | Усиливает `task-via-subagents`: результат шага подтверждается не только prose-report (текстовым отчётом), но и проверяемыми артефактами |
...
| Паттерн | Источники | Суть | Обоснование |
|---|---|---|---|
| **Typed I/O per step** (типизированный ввод/вывод шага) | Mastra AI (Zod), LangGraph (TypedDict), Archon (JSON Schema), Sandcastle (`Output.object/string` с Zod-валидацией), Agent Lifecycle Kit (public schema-backed receipts — публичные доказательства со схемами), bb Workflows (ограниченное подмножество JSON Schema) | Схемы валидации входных/выходных данных каждого шага | Повышает надёжность цепочек: невалидный input (ввод) → fail-fast (быстрый отказ). Подтверждено 6+ проектами |
| **Sub-agent pattern** | Claude Code (Task tool), Codex (spawn/wait/close_agent), OpenHands SDK (DelegateTool), Kilo Code (task tool: isolated session, permission inheritance, cost propagation, resume, parallel invocation), **OpenCode** (task tool: isolated session + permission inheritance + resume по task_id + no recursive delegation), **Herdr** (agent start/prompt/wait с фиксацией владельца панели), **bb Workflows** (`agent()`/`parallel()`, durable child threads) | Изолированный контекст подзадачи, потенциально параллельно | «Chain внутри chain» с собственным бюджетом и контекстом. Для dynamic chains. OpenCode добавляет resume по task_id; Herdr показывает атомарный prompt+wait и защиту от подмены агента; bb добавляет persisted call budget/replay (сохраняемый бюджет вызовов и воспроизведение) |
...
| **Parallel execution** | Archon (DAG layers), pi_agent_rust (read-only tools), Mastra AI (.parallel()), **Kilo Code** (wave-based: parallel tool calls в одном LLM-сообщении, dependency classification), **bb Workflows** (`parallel()` + bounded concurrency) | Параллельное выполнение независимых шагов | Оптимизация: lint + type-check + tests одновременно. Kilo Code добавляет wave-based подход, bb — сохраняемый FIFO scheduler (планировщик) с лимитом параллельности |
| **Per-step model override** | Archon (per-node provider/model), Mastra AI, Codex (custom agent roles), bb Workflows (`agent()` options) | Дешёвая модель для простых шагов, дорогая для сложных | Оптимизация стоимости: классификация → дешёвая модель, кодогенерация → сильная модель |
...
| **Checkpoint / Durable execution** | LangGraph (graph checkpoint + replay), bb Workflows (persisted calls + unchanged-prefix replay) | Сохранение состояния после шага/вызова, resume после сбоя | LangGraph даёт graph-level checkpointing; bb — устойчивый журнал вызовов и детерминированное воспроизведение префикса, но не доказывает идемпотентность побочных эффектов |
...
| **Human-in-the-loop** | Archon (approval nodes), Mastra AI (suspend/resume), GitHub Copilot Cloud Agent (plan review), Agno (3 режима: confirmation/user_input/output_review) | Пауза для подтверждения человеком в критических точках | Для production: LLM генерирует chain → человек подтверждает → оркестратор выполняет. ⚠️ В CLI ограниченно: только интерактивный режим |
| **Workflow nesting** | Mastra AI (chain как шаг другой chain), Agno (Workflow как Step, до 10 уровней), bb Workflows (один уровень child workflow) | Вложенные цепочки: реализация → (review + fix) → deploy | Композиция chain из переиспользуемых подцепочек; bb показывает полезность строгого ограничения глубины |
| **Git worktree isolation** | Archon (IIsolationProvider), Orca ADE, SwarmForge, **Herdr** (`worktree.create/open/remove` + provenance + events), **bb** (managed environment lifecycle + `.worktreeinclude`) | Каждый run в своём git worktree — параллельные runs не конфликтуют | Для параллельного выполнения цепочек в одном репозитории. Herdr и bb добавляют машинный lifecycle (жизненный цикл), но merge policy (политика слияния) остаётся внешней; автоочистка bb требует явного владельца |
...
* Duet: SKILL.md-формат для chain templates (frontmatter: runner, tools; body: prompt enrichment — де-факто стандарт, 28/36 проектов по критерию тренда ниже) — P3
...
* **ALK: frozen plan authority + lock digest** (замороженные полномочия плана + хеш замка: `plan.manifest.json` + `agent-plan-lock.v1`) — P2/P3
* **ALK: receipt-bound controller gates** (ворота контроллера, привязанные к доказательству `agent-controller-gate-receipt.v1`: run/task/attempt/operation/source/plan bindings — привязки запуска, задачи, попытки, операции, исходников и плана) — P3
* **ALK: implementation audit and final proof vocabulary** (словарь аудита реализации и финального доказательства: task result → independent review → implementation audit → completion/final proof) — P2/P3
* **ALK: adapter support-level taxonomy** (таксономия уровней поддержки адаптеров: `EXPERIMENTAL`/`VERIFIED`, точный диапазон CLI, live evidence summaries — сводки живых доказательств, граница запуска `WRAPPER_ONLY`) — P2
* **ALK: provider-neutral model classes** (нейтральные классы моделей `budget`/`standard-code`/`strong-reasoning` с конкретными host-local bindings — локальными привязками хоста) — P3
* **ALK: host env allowlist + redacted receipts** (список разрешённых переменных окружения хоста + отредактированные доказательства: `--host-env-allow`, редактирование секретов/локальных путей, без provider secrets — секретов поставщика — в переносимых артефактах) — P2
* **ALK: proof integrity hash chain** (хеш-цепочка целостности доказательств: stable finding id → root cause → fix impact → append-only receipt chain) — R&D

#### bb

* **bb: provider-neutral thread contract** (нейтральный контракт потока поставщика: общий lifecycle/events поверх Claude Code, Codex, Pi и ACP) — P2
* **bb: unified UI/CLI/API/SDK surfaces** (единые интерфейсы с общими идентификаторами и состояниями) — P2/P3
* **bb Workflows: durable call ledger + unchanged-prefix replay** (устойчивый журнал вызовов и воспроизведение неизменённого префикса после перезапуска) — P2 R&D перед реализацией идемпотентности
* **bb Workflows: structured result + bounded corrective turns** (JSON Schema-результат и ограниченные корректирующие ходы) — P2
* **bb: managed worktree ownership/cleanup** (управляемое владение и очистка рабочего дерева с `.worktreeinclude`) — P3, только с task/PR guards (защитными условиями)
* **bb: self-describing provider/UI/CLI/skills plugin** (самоописываемый плагин поставщика, интерфейса, команд и навыков) — R&D после trust/sandbox design (проектирования доверия и изоляции)

#### pragmatic-orchestration

* **pragmatic-orchestration: steer delivery lifecycle** (`accepted` ≠ `delivered` ≠ semantic application; mailbox `seq`, idempotent `client_id`, monotonic status/evidence ranks) — P1 R&D для интерактивного `AgentRunner`
* **pragmatic-orchestration: closed runner event schema** (неизвестные события отвергаются как protocol drift; raw/normalized/final provenance) — P1/P2
* **pragmatic-orchestration: fail-closed mode capability matrix** (filesystem/shell/web/memory/subagents/steer/interrupt + backend-specific enforcement) — P1/P2
* **pragmatic-orchestration: first-minute check + deviation journal** (ранняя содержательная проверка и журнал неожиданных отклонений для рискованных делегаций) — P2 quick win для `run-subagent`/`task-via-subagents`
* **pragmatic-orchestration: fake-CLI contract tests** (argv safety, schema drift, steering, cancellation, concurrency, recovery без реальных API) — P2
* **pragmatic-orchestration: native history locator + coverage summary** (read-only поиск без собственного индекса, ссылка до исходной записи, честная полнота сканирования) — P3 R&D
* **pragmatic-orchestration: multi-model review judge** (fan-out + deterministic union + LLM semantic verdict; судья не является независимым доказательством) — P2 R&D после измерения стоимости и качества

#### Strands Harness SDK

* **Strands: `TerminationReason` + raw provider reason** (типизированная причина завершения и исходная причина поставщика отдельно от retry classification) — P1
* **Strands: typed policy decisions** (`proceed`/`deny`/`guide`/`confirm` до эффекта, с пределом корректирующих циклов) — P1 R&D
* **Strands: low-level events + high-level policies** (низкоуровневые события отдельно от политик повтора, команд и качества) — P1/P2
* **Strands: explicit harness defaults and override precedence** (явный реестр дефолтов и однозначный приоритет переопределений) — P2
* **Strands: session / memory / context separation** (разные контракты возобновления, долговременной памяти и управления окном контекста) — P2 R&D
* **Strands: trace-/session-level eval taxonomy** (оценка результата, инструментов, траектории и восстановления на сохранённых трассах) — P2 R&D
* **Strands: two-layer observability** (runner telemetry + business orchestration audit с correlation IDs, redaction и политикой хранения) — P2
...
> Анализ выполнен на основе 39 / 39 подготовленных исследовательских артефактов; T3 Code #37, pragmatic-orchestration #38 и Strands Harness SDK #39 входят в расчёты. Тренды сгруппированы по значимости для архитектуры task-orchestrator.

### 1. Уникальная позиция task-orchestrator

**По 39 подготовленным исследовательским артефактам не найден проект с тем же полным набором:** chains (цепочки) + retry с backoff (повтор с задержкой) + circuit breaker (предохранитель отказов) + quality gates (ворота качества) + бюджетный контроль + `fix_iterations` + fallback routing (маршрутизация на запасной вариант). Это проверяемое отличие `task-orchestrator` в рамках текущей сводки, а не утверждение о всём рынке.

**В изученных проприетарных продуктах** (Claude Code, GitHub Copilot Cloud Agent, OpenAI Codex, Duet) не подтверждён тот же набор retry с backoff (повторов с задержкой), circuit breaker (предохранителя отказов), quality gates (ворот качества), budget limits (лимитов бюджета) и declarative chains (декларативных цепочек). Поэтому отличия `task-orchestrator` остаются релевантными и для сравнения с коммерческими AI-agent продуктами из этой сводки.
...
**20 из 39 подготовленных исследовательских артефактов** описывают базовую или first-class модель `LLM → tool call → observation → LLM → ...`. В numerator (числитель) входят: Crush (#1), pi_agent_rust (#2), CrewAI (#3), LangGraph (#4; agent loop может быть выражен как graph cycle/superstep), AutoGen (#5), OpenHands SDK (#6), MetaGPT (#8), OpenClaw (#9), Mastra AI (#10), Claude Code (#11), Copilot Cloud Agent (#12), Docker Agent + Codex (#13), Agno (#14), Hermes Agent (#18), Kilo Code (#21), OpenCode (#22), OmO (#23), Zeroclaw (#26), **Odysseus (#27)** и Strands Harness SDK (#39). Strands увеличивает числитель: SDK сам исполняет встроенный цикл модели и инструментов; собранный harness настраивает этот же цикл, а не делегирует его внешнему CLI. Ранее добавленные qm (#32), omnigent (#33), Herdr (#34), ALK (#35), bb (#36), T3 Code (#37) и pragmatic-orchestration (#38) не увеличивали numerator: они управляют внешними agent loops либо предоставляют lifecycle/control surfaces (поверхности управления). **bx-dev (#31)**, **Orca ADE (#30)** и **SwarmForge (#29)** также не увеличивали numerator: это соответственно manual workflow harness, ADE/manual orchestration layer и swarm-orchestration layer.

Не входят в agent-loop numerator: Archon (#7; DAG + subprocess SDK), Paperclip AI (#15; heartbeat-based мета-оркестрация), AgentCraft (#16; GUI wrapper), Factory Missions (#17; orchestrator-worker delegation), Oz (#19; cloud-managed runs), Sandcastle (#20; agent invocation loop в песочнице), Duet (#24; skill-driven orchestration), Multica (#25; daemon-based task dispatch), Agent Skills (#28; skill pack / prompt workflow library), qm (#32; platform-over-harnesses), omnigent (#33; meta-harness), SwarmForge (#29; tmux-based swarm orchestration platform), Orca ADE (#30; desktop/mobile manual parallel-agent harness), bx-dev (#31; Codex-skill/manual workflow harness), Herdr (#34; terminal agent runtime / control surface), Agent Lifecycle Kit (#35; lifecycle/evidence controller), bb (#36; agentic IDE/control surface с опциональным workflow engine), T3 Code (#37; GUI control surface над внешними харнесами), pragmatic-orchestration (#38; skill+CLI orchestration layer над внешними харнесами). **Duet** (#24) использует skill-driven orchestration: intent → skill selection → multi-phase autonomous execution. **Multica** (#25) использует daemon-based task dispatch (не agent loop, а task queue).
...
**bb не учитывается в этом подсчёте:** он нормализует и запускает внешние agent loops через provider plugins; даже Workflows делегирует рассуждение обычным bb threads, а не вызывает LLM API самостоятельно.

**Agent Lifecycle Kit не учитывается в этом подсчёте:** это lifecycle/evidence controller (контроллер жизненного цикла и доказательств) над внешними host CLIs (командными хостами). ALK фиксирует plan/state/receipts/audits and next action (план, состояние, доказательства, аудиты и следующее действие), но не вызывает модель и не реализует `LLM → tool call → observation` loop (цикл «модель → вызов инструмента → наблюдение»).
...
### 3. Разделение на восемнадцать уровней абстракции

**Все 39 подготовленных исследовательских артефактов** чётко делятся на восемнадцать уровней:

| Уровень | Проекты | Что делают | Аналог в task-orchestrator |
|---|---|---|---|
| **SDK / Agent runtime** | Crush, pi_agent_rust, OpenHands SDK, Mastra AI, Claude Code, Codex, OpenClaw, Agno, Hermes Agent, Kilo Code, **OpenCode**, **OmO**, **Zeroclaw**, **Odysseus**, **Strands Harness SDK** | Работают на уровне прямых LLM API | Runner'ы (pi, codex) |
| **Оркестратор / Workflow engine** | CrewAI, LangGraph, AutoGen, Archon, MetaGPT, GitHub Copilot Cloud Agent | Управляют потоком выполнения между агентами/шагами | Chain executor |
...
| **Agentic IDE / Programmable control surface** (агентная среда разработки и программируемая управляющая поверхность) | bb, T3 Code | Нормализует внешних поставщиков в threads (потоки); bb добавляет UI/CLI/API/SDK, managed worktrees и опциональный durable Workflows plugin, T3 Code — server-owned event journal, command receipts и capability-first provider contract | Потенциальная ручная поверхность и источник provider/thread/replay/capability patterns (паттернов поставщиков, потоков, воспроизведения и возможностей); не core dependency |
| **Lifecycle / Evidence controller** (контроллер жизненного цикла и доказательств) | Agent Lifecycle Kit | Держит reviewed/frozen plan (проверенный/замороженный план), state (состояние), receipts (доказательства), audits (аудиты), proof (финальное доказательство), adapter support claims (заявления поддержки адаптеров) и managed next actions (управляемые следующие действия) вокруг external host CLIs (внешних командных хостов) | Потенциальный governance layer (слой управления доказательствами) поверх `task-via-subagents`; не runtime (не среда выполнения) |
...
| **Multiplayer / Multi-tenant agent platform** | qm | Platform-over-harnesses: per-employee scopes, Slack/web/admin surfaces, durable per-scope sandbox, shared skills governance и per-turn routing во внешние harness'ы | Потенциальный control-plane слой вокруг runner-профилей; прямого аналога нет |
| **Meta-harness / multi-device collaboration platform** (мета-харнес и платформа совместной работы с разных устройств) | omnigent | Единый harness-контракт над 12+ внешними coding-агентами (native tmux/PTY + SDK) и custom YAML; поверх — policies (server/agent/session), cloud + OS sandboxing, multi-agent supervision, collaboration и跨-device surfaces | `AgentRunnerInterface` + `config/chains.yaml` (runner-abstraction, частичный аналог); collaboration/multi-device — нет аналога |
| **Orchestration skill suite / delegation layer** (набор навыков оркестрации и слой делегации) | pragmatic-orchestration | Agent Skills + один `porch` CLI: read-only review fan-out, один steerable delegate, sessions/quota, закрытые события и файловый mailbox поверх внешних харнесов | `run-subagent` + `task-via-subagents` + `AgentRunner` (частичные аналоги); живого steer и общей capability matrix нет |
...
**qm** добавляет уровень **Multiplayer / Multi-tenant agent platform**: это platform-over-harnesses (платформа поверх харнесов) для командной работы, где ядро управляет identity/scope/delivery/security/skills, а собственный agent loop остаётся во внешних harness'ах.

**pragmatic-orchestration** добавляет отдельный уровень **Orchestration skill suite / delegation layer**: это не agent loop и не chain engine, а переносимая инструкция для родительского агента плюс тонкий CLI над внешними харнесами. В отличие от qm/omnigent, слой не строит multi-tenant или collaboration-платформу; в отличие от Herdr/bb/T3 Code, не требует постоянной IDE/PTY/server surface. Его специализация — per-run supervisor, steer mailbox/evidence, review fan-out и дисциплина приёмки.

**omnigent** добавляет уровень **Meta-harness / multi-device collaboration platform**: это не coding agent и не chain engine, а общий оркестрационный слой над внешними coding-агентами (harness abstraction — единый контракт, swap/combine без переписывания) с policies, cloud + OS sandboxing, multi-agent supervision и跨-device collaboration поверх него. omnigent — ближайший аналог qm (#32, тоже meta-harness над внешними агентами), но шире по harness-покрытию (12+ харнесов + custom YAML) и sandboxing (OS + 11 cloud providers), богаче по policy engine (function/CEL, server/agent/session, spend caps); слабее в multi-tenant/org-governance, где силён qm. Retry/backoff/error-classification есть только на микроуровне (LLM/tool-call внутри agent loop), не chain/runner.

### 4. SKILL.md / AGENTS.md — де-факто стандарт

**Критерий подсчёта:** проект учитывается, если его индивидуальный отчёт явно подтверждает поддержку переиспользуемых инструкций для возможностей агента через `SKILL.md`, каталог навыков поставщика (`.claude/skills/`, `.codex/skills/` и аналоги) либо отдельный файловый пакет навыков с обнаружением или подключением хостом. Общие инструкции репозитория (`AGENTS.md`, `CLAUDE.md`), навыки только как сущности базы данных или пользовательского интерфейса и недокументированные маркетинговые упоминания сами по себе не учитываются. Каждый проект учитывается не более одного раза.

**30 из 39 подготовленных исследовательских артефактов** соответствуют этому критерию. Ровно учтённые проекты:
1. Crush (#1)
2. pi_agent_rust (#2)
3. CrewAI (#3)
4. OpenHands SDK (#6)
...
6. OpenClaw (#9)
7. Mastra AI (#10)
8. Claude Code (#11; `.claude/skills/`)
9. Docker Agent + OpenAI Codex (#13; `SKILL.md`, `.codex/skills/`)
10. Agno (#14)
11. Factory Missions (#17; `.factory/skills/`)
12. Hermes Agent (#18)
13. Oz (Warp) (#19; `oz-skills`)
14. Kilo Code (#21)
15. OpenCode (#22; `.opencode/skills/`, `.claude/skills/`, `.agents/skills/`)
16. Oh My OpenAgent (#23; Skill-Embedded MCPs в `SKILL.md`)
17. Duet (#24; реестр `duet-skills` с `SKILL.md`)
18. Multica (#25; навыки рабочей области в `SKILL.md`)
19. Zeroclaw (#26; SkillForge и `SKILL.md`)
20. Odysseus (#27; `SKILL.md` в `memory/skills`)
21. Agent Skills (#28; 24 файла `SKILL.md`)
22. Orca ADE (#30; устанавливаемые Orca Skills)
23. bx-dev (#31; `skills/bx-dev/SKILL.md` и 105 вспомогательных навыков)
24. qm (#32; пакеты навыков и deployment skills)
25. omnigent (#33; потребляет `.claude/skills/`)
26. Herdr (#34; `skills/herdr/SKILL.md`)
27. Agent Lifecycle Kit (#35; 7 файлов `skills/*/SKILL.md`)
28. bb (#36; встроенные и подключаемые `SKILL.md`)
29. pragmatic-orchestration (#38; два распространяемых Agent Skills, включая основной навык `pragmatic-orchestration`)
30. Strands Harness SDK (#39; `AgentSkills` загружает каталоги с `SKILL.md` в формате agentskills.io, harness по умолчанию ищет `./.agent/skills`)

**T3 Code не увеличивает числитель SKILL.md / AGENTS.md:** его индивидуальный отчёт не подтверждает распространяемые файловые инструкции или пакет навыков, соответствующие критерию подсчёта; упоминания `SKILL.md` и `AGENTS.md` относятся к сопоставлению с task-orchestrator.

Типичный формат — YAML frontmatter + markdown body, обнаружение из нескольких мест и валидация.
...
- **omnigent** (#33) — и consumer (потребляет `.claude/skills/` + `AGENTS.md`), и producer (custom-агенты в YAML: `.github/agents/*.yaml`, `examples/*/skills/`); комбинация skill-consumer + agent-producer в одном продукте

**AGENTS.md** используется как минимум в 12 проектах (Crush, pi_agent_rust, OpenHands SDK, Codex, Paperclip AI, Factory Missions (per-mission AGENTS.md с boundaries + guidance), Hermes Agent (`.hermes.md` / `HERMES.md`), Agent Skills, Orca ADE, **omnigent**, **qm**, task-orchestrator) — де-факто стандарт для AI-agent контекста.

### 5. MCP (Model Context Protocol) — повсеместный протокол расширения

**21 из 39 подготовленных исследовательских артефактов** подтверждают поддержку MCP:
- Crush, CrewAI, OpenHands SDK, Archon, OpenClaw, Mastra AI, Claude Code, Copilot Cloud Agent, Codex, Agno, Paperclip AI, Hermes Agent, Oz (Warp), Kilo Code, **OpenCode** (full MCP client: tools + resources + OAuth), **OmO** (Skill-Embedded MCPs: MCP-серверы внутри скиллов, per-session изоляция), **Zeroclaw** (MCP client: подключение внешних MCP-серверов как инструментов), **Orca ADE** (MCP endpoints under Settings → Integrations → MCP; tools appear inside compatible agent CLIs), **qm**, **omnigent** (custom-agent YAML `tools: {type: mcp}`, server `mcp_pool.py`, runner `mcp_manager.py`), **Strands Harness SDK** (MCP-клиенты подключаются как инструменты SDK и конфигурируются собранным harness)
...
**pragmatic-orchestration не увеличивает числитель MCP:** `porch` не реализует собственный MCP client/server; возможности подключённых харнесов не приписываются управляющему слою. **Herdr не увеличивает числитель MCP:** его расширения используют CLI/socket API и исполняемые плагины. Это независимая управляющая поверхность, а не MCP client/server (клиент/сервер MCP). **bb не увеличивает числитель MCP:** собственное MCP client/server ядро bb не подтверждено; MCP-возможности подключённых внешних агентов не приписываются управляющей поверхности. **ALK также не увеличивает числитель MCP:** он проверяет adapter/host capability surfaces и может импортировать локальный external-memory export, но не является MCP client/server. **omnigent увеличивает числитель MCP:** custom-agent YAML поддерживает `tools: {type: mcp}`, а server/runner держат MCP-pool/manager для подмены инструментов агентам.

### 6. Контекст-менеджмент — повсеместная проблема

**15 из 39 подготовленных исследовательских артефактов** подтверждают auto-compaction / auto-summarization при context overflow:
- Crush, pi_agent_rust, OpenHands SDK, Mastra AI, Claude Code, Codex, Agno, Hermes Agent, Kilo Code, **OpenCode** (7-секционный structured template + pruning + preserve recent turns + replay), **OmO** (proactive compaction: monitor + compact **до** ошибки + dynamic context pruning: dedup/supersede/purge), **Zeroclaw** (ContextCompressor: LLM-суммаризация + HistoryPruner: orphaned tool messages + collapsed pairs + protected indices + 1.2x token safety margin), **qm** (`context-compaction.ts`), **omnigent** (`runtime/compaction.py`), **Strands Harness SDK** (`context_manager="auto"`: обрезка крупных результатов инструментов и суммаризация старых сообщений при 85% окна)
- bb, T3 Code и pragmatic-orchestration не увеличивают числитель: собственное универсальное auto-compaction ядро у этих управляющих слоёв не подтверждено; provider-native memory/compaction не приписывается поверхности
- Все учтённые проекты используют LLM-суммаризацию для сжатия истории
...
**bb не входит в восемь продвинутых моделей безопасности:** Workflows изолирован в QuickJS и прямой bind требует явного включения, но обычные плагины и агенты выполняются с правами пользователя, а прямой HTTP API неаутентифицирован.
...
**pragmatic-orchestration также не увеличивает числитель:** его fail-closed capability matrix и backend-specific read-only флаги — сильный policy pattern (паттерн политики), но авторы прямо фиксируют отсутствие собственной песочницы, а delegate работает с полным доступом.
...
**26 из 39 подготовленных runtime/platform-level исследовательских артефактов** подтверждают sub-agents или multi-agent на уровне runtime (среды выполнения) или platform-level (уровне платформы). Orca ADE входит в числитель как manual parallel-agent orchestration (ручная параллельная оркестрация агентов), bx-dev — как Codex-skill orchestration of single-shot subagents (оркестрация одноразовых сабагентов через навык Codex), Herdr — как агентная управляющая поверхность, где агент может создать панель, запустить именованного внешнего агента, отправить ему промпт и ждать состояния, qm — как platform-level multi-agent supervision (платформенный надзор над несколькими внешними harness'ами/scopes), **omnigent** — как meta-harness с first-class multi-agent supervision (под-агенты в YAML `type: agent`, cross-vendor review, roster preflight, Polly с 7 sub-agents), **T3 Code** — как параллельные потоки внешних харнесов, **pragmatic-orchestration** — как параллельный review fan-out с одним родительским агрегатором, **Strands Harness SDK** — как среда выполнения с агентом как инструментом и A2A, а также `Graph` и `Swarm` с состоянием и ограничителями выполнения, передач и времени; A2A внутри `Swarm` пока не поддерживается. ALK консервативно вынесен из числителя: он координирует review/worker artifacts (артефакты ревью и исполнителей), но core (ядро) не запускает ревьюеров автоматически.

Воспроизводимый список 26 учтённых проектов:
...
20. Herdr — agent-aware terminal runtime (терминальная среда выполнения, понимающая агентов): `agent start/prompt/wait/read`, несколько независимых PTY, фиксация текущего владельца панели и worktree lifecycle (жизненный цикл рабочего дерева).
21. qm — platform-level multi-agent supervision: per-employee scopes, approved harness-router для Pi/OpenCode/Codex/Claude Code, Slack/web/admin coordination и shared skills governance.
22. omnigent — meta-harness multi-agent supervision (надзор): под-агенты в YAML (`type: agent`), cross-vendor review (дифф исполнителя ревьюит агент другого вендора), roster preflight (проверка доступности CLI до dispatch), `spawn: true` (создание child-сессий); Polly — 7 sub-agents (Claude Code/Codex/OpenCode/Cursor/Hermes/Antigravity/Pi) с параллельными worktrees.

23. bb — параллельные provider-neutral threads (нейтральные потоки поставщиков), child threads (дочерние потоки) и Workflows `agent()`/`parallel()` с ограничениями concurrency/calls (параллельности/вызовов).
24. T3 Code — параллельные долговечные потоки внешних харнесов, которыми владеет серверная управляющая поверхность.
25. pragmatic-orchestration — параллельный fan-out независимых review-проходов и родительская агрегация; delegate при этом выбирает ровно одного агента.
26. Strands Harness SDK — агент как инструмент и A2A без указанных ограничителей; `Graph` и `Swarm` с состоянием и ограничителями выполнения, передач и времени. A2A внутри `Swarm` пока не поддерживается.

Agent Lifecycle Kit — смежный паттерн host-owned multi-review/worker coordination (координации ревью и исполнителей на стороне хоста): Review Mesh assignments/import/synthesis/quorum (назначения, импорт, синтез и кворум) и task packets (пакеты задач) для external host CLIs (внешних командных хостов). Core (ядро) ALK не запускает ревьюеров по умолчанию, поэтому в числитель не включён.
...
- LangGraph (conditional edges), Archon (`when:` expressions), Mastra AI (`.branch()`), GitHub Copilot Cloud Agent (plan branching), Agno (Condition + Router)
...
**6 проектов** используют типизированные схемы для I/O шагов/доказательств:
- Mastra AI (Zod-схемы), LangGraph (TypedDict + reducers), Archon (JSON Schema), Sandcastle (Zod structured output — структурированный вывод Zod), Agent Lifecycle Kit (schema-backed public receipts and CLI JSON envelopes — публичные доказательства и JSON-конверты CLI со схемами), bb Workflows (ограниченное подмножество JSON Schema + Ajv-валидация результата)
...
**8 проектов** классифицируют ошибки для умного retry:
- Archon (FATAL/TRANSIENT/UNKNOWN), OpenClaw (6 категорий по HTTP status), Codex (Guardian risk taxonomy), Hermes Agent (20+ FailoverReason: auth, billing, rate_limit, overloaded, server_error, timeout, context_overflow, model_not_found, provider_policy_blocked и др.), **Kilo Code** (ContextOverflow, API 5xx, rate limit, FreeUsageLimitError, Kilo errors — retry/no-retry classification), **OpenCode** (ContextOverflow/API 5xx/rate limit/FreeUsageLimitError/GoUsageLimitError — retry/no-retry + Retry-After header parsing + doom loop detection), **Zeroclaw** (retryable: context window errors, 429, 408; non-retryable: 4xx except 429/408, auth failures, model not found + context limit auto-detection из error messages), **bb Workflows** (явный SDK `retryable` marker либо консервативные overload/rate-limit/provider-5xx/network signatures; auth/config/schema errors не повторяются)
...
|---|---|---|
| **Python** | CrewAI, LangGraph, AutoGen, OpenHands SDK, MetaGPT, Agno, Hermes Agent, **bx-dev** (support scripts; primary artifact is Markdown skill), **Agent Lifecycle Kit**, **pragmatic-orchestration** (Python + Bash; primary artifact is Agent Skills + CLI) | Доминирующий язык для AI-agent фреймворков и support tooling |
| **TypeScript** | Archon, OpenClaw, Mastra AI, Paperclip AI, Sandcastle, Kilo Code, **OpenCode**, **Multica** (frontend), Orca ADE, **bb**, **T3 Code** | Растущая экосистема, особенно для workflow engines, мета-оркестраторов, ADE и sandbox orchestration |
...
* **LangGraph** остаётся наиболее явным graph-level durable execution (устойчивым исполнением уровня графа) с checkpoint persistence. **bb Workflows** добавляет иной подтверждённый вариант: SQLite-журнал вызовов, сохраняемый retry budget и unchanged-prefix replay (воспроизведение неизменённого префикса), но успешный replay не доказывает повторяемость файловых побочных эффектов. LangGraph требует `langchain-core` как обязательную зависимость — не standalone.
...
* **Agent Lifecycle Kit — первый lifecycle/evidence controller (контроллер жизненного цикла и доказательств) в исследовании:** Python package and CLI (пакет Python и интерфейс командной строки) вокруг внешних coding agents (агентов программирования) с frozen plan (замороженным планом), schema-backed receipts (доказательствами со схемами), implementation audit (аудитом реализации), final proof (финальным доказательством), adapter support levels (уровнями поддержки адаптеров), model class routing (маршрутизацией классов моделей) и host-local launch profiles (локальными профилями запуска хоста). Это не coding agent и не chain engine: host CLIs (командные хосты) выполняют модельную работу, а ALK сохраняет authority/proof (полномочия и доказательства). Вердикт: не core dependency (не основная зависимость), но источник patterns (паттернов) для frozen authority, receipt-bound gates, adapter support taxonomy, proof integrity and env redaction (замороженных полномочий, ворот по доказательствам, таксономии поддержки адаптеров, целостности доказательств и редактирования окружения).
* **bb — первая agentic IDE / programmable control surface (агентная среда разработки и программируемая управляющая поверхность) с подтверждённым опциональным durable workflow engine (устойчивым движком процессов) в исследовании:** provider-neutral threads, UI/CLI/HTTP/WebSocket/TypeScript SDK, managed worktrees и self-describing plugins объединены с отключённым по умолчанию QuickJS Workflows. Workflows сохраняет вызовы в SQLite, воспроизводит успешный неизменённый префикс, ограничивает concurrency/calls/timeout и валидирует structured output (структурированный вывод), но не даёт chain-level circuit breaker, shell quality gates, cost budget или `fix_iterations`. Вердикт: не core dependency; заимствовать provider/thread/durable-replay patterns; допустим как опциональная ручная среда после проверки ownership/security.
...
* **T3 Code — управляющая поверхность с server-owned durable threads (долговечными потоками сервера):** отделяет поток от заменяемой сессии харнеса, фиксирует намерение до I/O, проверяет capability до эффекта и связывает execution с worktree/checkpoint. Вердикт: заимствовать capability-контракт и BYO subscription; отдельно изучить долговечные потоки, worktree, permissions и remote; не переносить one-button PR в движок.
* **pragmatic-orchestration — первый специализированный skill+CLI-слой с подтверждённым steer mailbox/evidence lifecycle:** один `porch` нормализует review/delegate/sessions/quota над внешними CLI, отделяет локальный приём steer от backend delivery и semantic application, закрывает event/mode schemas и формализует родительскую супервизию. Это не chain engine и не sandbox: нет chain-level retry/CB/gates/budget, delegate работает с полным доступом. Вердикт: не dependency; заимствовать steer/event/policy/deviation/fake-CLI patterns, review judge и session analytics изучать отдельно.
* **Strands Harness SDK — первый SDK трека, у которого двуязычный паритет Python/TypeScript дополнен отдельным собранным харнесом и SDK оценивания:** Python/TypeScript runtime владеет циклом `model → tool → model`, формализует лимиты и stop reasons, hooks/interventions, sessions/memory/context, `Graph`/`Swarm`/A2A и OpenTelemetry. `create_harness()` добавляет явные переопределяемые дефолты, Agent Skills и MCP. Вердикт: не dependency для PHP-оркестратора внешних CLI; заимствовать termination/policy/defaults/state/observability/eval patterns, а tool-level interception и trace evaluation исследовать отдельно.
...
| 2026-08-12 | Аналитик (Шерлок) | Создан отчёт agent-lifecycle-kit-comparison.md, заполнена строка Agent Lifecycle Kit (#35). Статус обновлён до 33 завершённых / 35 запланированных: #32 qm и #33 omnigent всё ещё ожидают исследования. Пересчитаны тренды: agent loop 19/33 (ALK не execution layer), SKILL.md 24/33, MCP 18/33, sub-agents/multi-agent 20/33 (ALK вынесен в смежный паттерн), compression 12/33, typed/schema-backed I/O 5 проектов. Добавлен четырнадцатый уровень `Lifecycle / Evidence controller` и рекомендации: frozen plan authority, receipt-bound controller gates, implementation audit/final proof vocabulary, adapter support-level taxonomy, provider-neutral model classes, host env allowlist and proof integrity hash chain. |
| 2026-08-29 | Аналитик (Шерлок) | Создан `bb-comparison.md`, заполнена строка bb (#36), набор исследовательских артефактов обновлён до 36/36. Пересчитаны затронутые тренды: agent loop 19/36 (bb не реализует собственный LLM/tool loop), SKILL.md 28/36, MCP 20/36, sub-agents/multi-agent 23/36, compression 14/36, typed/schema-backed I/O 6 проектов, error classification 8 проектов. Добавлен семнадцатый уровень `Agentic IDE / Programmable control surface`; зафиксированы provider-neutral threads, managed worktrees, unified surfaces и отключённый по умолчанию durable QuickJS Workflows plugin. Verdict: 🟡 паттерны / 🔴 не core dependency / 🟢 optional manual environment после проверки ownership/security. |
| 2026-08-29 | Аналитик (Шерлок) | По результатам самопроверки уточнён статус: все 36/36 исследовательских артефактов подготовлены и участвуют в расчётах, при этом TASK bb остаётся `in_progress` до процессной приёмки. Исправлено количество ближайших аналогов с учётом ALK. |
| 2026-08-29 | Аналитик (Шерлок) | Повторная самопроверка: в классификацию 17 уровней добавлен пропущенный Odysseus (#27), название GitHub Copilot Cloud Agent приведено к каноническому, подтверждено однократное распределение всех 36 позиций. Для тренда SKILL.md зафиксирован воспроизводимый критерий и нумерованный список; корректный результат — 28/36, включая ранее пропущенные в перечне Claude Code (#11) и omnigent (#33). |
| 2026-08-29 | Аналитик (Шерлок) | Доработка по замечаниям Ревьювера Бэка Пуаро: вывод по MCP упрощён до доказанного отсутствия подтверждённого собственного MCP client/server ядра bb; отчёт дополнен первичными доказательствами Ajv и политики повторов, канонической шапкой и snapshot-ссылкой на перечень поставщиков. |
| 2026-08-29 | Аналитик (Шерлок) | После процессной приёмки и зелёного CI приняты все 36/36 исследовательских артефактов; TASK bb переведена в `done`, актуальные шапка статуса, вводный вывод и преамбула трендов синхронизированы с завершённым состоянием. |
| 2026-09-22 | Аналитик (Шерлок) | Создан `pragmatic-orchestration-comparison.md`, заполнена строка #38 и сводка обновлена до 38/38. Одновременно устранён накопленный в commit `cc694ecf` рассинхрон T3 Code #37: при шапке 37/37 тренды оставались на 36/36; изменения T3 Code синхронизированы вместе с добавлением #38. Пересчитаны затронутые тренды: agent loop 19/38, SKILL.md 29/38, MCP 20/38, compression 14/38, sub-agents/multi-agent 25/38. Добавлен восемнадцатый уровень `Orchestration skill suite / delegation layer`; зафиксированы steer mailbox/evidence, closed events, fail-closed mode capabilities, deviation journal, session locators и fake-CLI tests. Verdict: 🟡 паттерны / 🔴 не dependency. |
| 2026-09-23 | Аналитик (Шерлок) | Создан `strands-harness-sdk-comparison.md`, заполнена строка #39 и сводка обновлена до 39/39. Пересчитаны затронутые тренды: agent loop 20/39, SKILL.md 30/39, MCP 21/39, compression 15/39, sub-agents/multi-agent 26/39. Зафиксированы typed stop reasons, hooks/interventions, разделение session/memory/context, harness defaults, Evals SDK и двухуровневая observability. Verdict: 🟡 паттерны / 🔴 не dependency. |
