...
> **Статус заполнения:** 33 завершённых / 35 запланированных исследований. Номера #32 (`qm`) и #33 (`omnigent`) зарезервированы активными задачами; Herdr #34 и Agent Lifecycle Kit #35 завершены раньше них.
...
> Анализ выполнен на основе 33 завершённых исследований. `qm` #32 и `omnigent` #33 запланированы, но до их завершения не входят в расчёты. Тренды сгруппированы по значимости для архитектуры task-orchestrator.

### 1. Уникальная позиция task-orchestrator

**По 33 завершённым исследованиям не найден проект с тем же полным набором:** chains (цепочки) + retry с backoff (повтор с задержкой) + circuit breaker (предохранитель отказов) + quality gates (ворота качества) + бюджетный контроль + `fix_iterations` + fallback routing (маршрутизация на запасной вариант). Это проверяемое отличие `task-orchestrator` в рамках текущей сводки, а не утверждение о всём рынке.
...
**19 из 33 завершённых исследований** используют базовую или first-class модель `LLM → tool call → observation → LLM → ...`. В numerator (числитель) входят: Crush (#1), pi_agent_rust (#2), CrewAI (#3), LangGraph (#4; agent loop может быть выражен как graph cycle/superstep), AutoGen (#5), OpenHands SDK (#6), MetaGPT (#8), OpenClaw (#9), Mastra AI (#10), Claude Code (#11), Copilot Cloud Agent (#12), Docker Agent + Codex (#13), Agno (#14), Hermes Agent (#18), Kilo Code (#21), OpenCode (#22), OmO (#23), Zeroclaw (#26), **Odysseus (#27)**. Herdr (#34) увеличил denominator (знаменатель) до `32`, а ALK (#35) — до `33`, но оба не увеличивают numerator: Herdr запускает внешние agent loops (циклы агентов) в реальных терминалах, а ALK управляет lifecycle/proof вокруг внешних host CLIs и не реализует собственный LLM/tool loop. Ранее добавленные **bx-dev (#31)**, **Orca ADE (#30)** и **SwarmForge (#29)** также не увеличивали numerator: это соответственно manual workflow harness, ADE/manual orchestration layer и swarm-orchestration layer.
...
**Все 33 завершённых исследования** чётко делятся на четырнадцать уровней:
...
**24 из 33 завершённых исследований** используют SKILL.md или аналогичный формат для формализации agent capabilities:
...
**18 из 33 завершённых исследований** поддерживают MCP:
...
**12 из 33 завершённых исследований** реализуют auto-compaction / auto-summarization при context overflow:
...
**20 из 33 завершённых runtime/platform-level исследований** поддерживают sub-agents или multi-agent на уровне runtime (среды выполнения) или platform-level (уровне платформы). Orca ADE входит в числитель как manual parallel-agent orchestration (ручная параллельная оркестрация агентов), bx-dev — как Codex-skill orchestration of single-shot subagents (оркестрация одноразовых сабагентов через навык Codex), Herdr — как агентная управляющая поверхность, где агент может создать панель, запустить именованного внешнего агента, отправить ему промпт и ждать состояния. ALK консервативно вынесен из числителя: он координирует review/worker artifacts (артефакты ревью и исполнителей), но core (ядро) не запускает ревьюеров автоматически.
