...
> **Статус заполнения:** 34 завершённых / 35 запланированных исследований. Номер #33 (`omnigent`) ещё не завершён; Herdr #34 и Agent Lifecycle Kit #35 завершены раньше #32/#33.
...
| Паттерн | Источники | Суть | Обоснование |
|---|---|---|---|
| **Auto-compaction / summarization** | Crush, pi_agent_rust, OpenHands SDK, Mastra AI, Claude Code, Codex, Agno, Hermes Agent, Kilo Code, **OpenCode** (7-секционный structured summary template: Goal/Constraints/Progress/Key Decisions/Next Steps/Critical Context/Relevant Files + pruning + preserve recent turns + replay), OmO, Zeroclaw, qm (`context-compaction.ts`) | LLM-суммаризация при context overflow | Широко распространённый паттерн (13/34). Для длинных цепочек и dynamic loops. OpenCode добавляет pruning, preserve recent и replay — три дополнительных механизма. OmO добавляет proactive compaction; qm подтверждает platform-level compaction для multi-tenant harness-сессий |
...
> Анализ выполнен на основе 34 завершённых исследований. Только `omnigent` #33 остаётся незавершённым и не входит в расчёты. Тренды сгруппированы по значимости для архитектуры task-orchestrator.
...
**19 из 34 завершённых исследований** (56%) используют базовую или first-class модель `LLM → tool call → observation → LLM → ...`. В numerator (числитель) входят: Crush (#1), pi_agent_rust (#2), CrewAI (#3), LangGraph (#4; agent loop может быть выражен как graph cycle/superstep), AutoGen (#5), OpenHands SDK (#6), MetaGPT (#8), OpenClaw (#9), Mastra AI (#10), Claude Code (#11), Copilot Cloud Agent (#12), Docker Agent + Codex (#13), Agno (#14), Hermes Agent (#18), Kilo Code (#21), OpenCode (#22), OmO (#23), Zeroclaw (#26), **Odysseus (#27)**. Herdr (#34) увеличил denominator (знаменатель) до `32`, ALK (#35) — до `33`, а qm (#32) — до `34`, но все трое не увеличивают numerator: Herdr запускает внешние agent loops (циклы агентов) в реальных терминалах, а ALK управляет lifecycle/proof вокруг внешних host CLIs и не реализует собственный LLM/tool loop. qm направляет один turn (ход) в выбранный внешний harness (Pi/OpenCode/Codex/Claude Code) и не реализует собственный agent loop. Ранее добавленные **bx-dev (#31)**, **Orca ADE (#30)** и **SwarmForge (#29)** также не увеличивали numerator: это соответственно manual workflow harness, ADE/manual orchestration layer и swarm-orchestration layer.
...
### 3. Разделение на пятнадцать уровней абстракции
...
| **Multiplayer / Multi-tenant agent platform** | qm | Даёт per-employee scopes (области сотрудников), Slack/web/admin surfaces (поверхности), durable sandbox (устойчивую песочницу), shared skills governance (общее управление навыками) и per-turn routing (маршрутизацию хода) во внешние harness'ы | Потенциальный platform/control-plane слой вокруг runner-профилей; прямого аналога нет |
...
**Herdr не увеличивает числитель MCP:** его расширения используют CLI/socket API и исполняемые плагины. Это независимая управляющая поверхность, а не MCP client/server (клиент/сервер MCP). **ALK также не увеличивает числитель MCP:** он проверяет adapter/host capability surfaces и может импортировать локальный external-memory export, но не является MCP client/server.

### 6. Контекст-менеджмент — повсеместная проблема

**13 из 34 завершённых исследований** (38%) реализуют auto-compaction / auto-summarization при context overflow:
- Crush, pi_agent_rust, OpenHands SDK, Mastra AI, Claude Code, Codex, Agno, Hermes Agent, Kilo Code, **OpenCode** (7-секционный structured template + pruning + preserve recent turns + replay), **OmO** (proactive compaction: monitor + compact **до** ошибки + dynamic context pruning: dedup/supersede/purge), **Zeroclaw** (ContextCompressor: LLM-суммаризация + HistoryPruner: orphaned tool messages + collapsed pairs + protected indices + 1.2x token safety margin), **qm** (`context-compaction.ts` для сжатия контекста harness-сессий)
...
**21 из 34 завершённых runtime/platform-level исследований** (62%) поддерживают sub-agents или multi-agent на уровне runtime (среды выполнения) или platform-level (уровне платформы). Orca ADE входит в числитель как manual parallel-agent orchestration (ручная параллельная оркестрация агентов), bx-dev — как Codex-skill orchestration of single-shot subagents (оркестрация одноразовых сабагентов через навык Codex), Herdr — как агентная управляющая поверхность, где агент может создать панель, запустить именованного внешнего агента, отправить ему промпт и ждать состояния, qm — как platform-level multi-agent supervision (платформенный надзор над несколькими внешними harness'ами/scopes). ALK консервативно вынесен из числителя: он координирует review/worker artifacts (артефакты ревью и исполнителей), но core (ядро) не запускает ревьюеров автоматически.

Воспроизводимый список 21 учтённого проекта:
