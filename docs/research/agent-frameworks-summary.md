...
> **Статус заполнения:** 35 завершённых / 35 запланированных исследований. `qm` #32 и `omnigent` #33 завершены; Herdr #34 и Agent Lifecycle Kit #35 также завершены.
...
| Паттерн | Источники | Суть | Обоснование |
|---|---|---|---|
| **Typed I/O per step** (типизированный ввод/вывод шага) | Mastra AI (Zod), LangGraph (TypedDict), Archon (JSON Schema), Sandcastle (`Output.object/string` с Zod-валидацией), Agent Lifecycle Kit (public schema-backed receipts — публичные доказательства со схемами) | Схемы валидации входных/выходных данных каждого шага | Повышает надёжность цепочек: невалидный input (ввод) → fail-fast (быстрый отказ). Подтверждено 5+ проектами |
...
> Анализ выполнен на основе 35 завершённых исследований; `qm` #32 и `omnigent` #33 завершены и входят в расчёты. Тренды сгруппированы по значимости для архитектуры task-orchestrator.

### 1. Уникальная позиция task-orchestrator

**По 34 завершённым исследованиям не найден проект с тем же полным набором:** chains (цепочки) + retry с backoff (повтор с задержкой) + circuit breaker (предохранитель отказов) + quality gates (ворота качества) + бюджетный контроль + `fix_iterations` + fallback routing (маршрутизация на запасной вариант). Это проверяемое отличие `task-orchestrator` в рамках текущей сводки, а не утверждение о всём рынке.
...
### 3. Разделение на шестнадцать уровней абстракции

**Все 34 завершённых исследования** чётко делятся на пятнадцать уровней:
...
**20 из 35 завершённых исследований** поддерживают MCP:
...
**5 проектов** используют типизированные схемы для I/O шагов/доказательств:
- Mastra AI (Zod-схемы), LangGraph (TypedDict + reducers), Archon (JSON Schema), Sandcastle (Zod structured output — структурированный вывод Zod), Agent Lifecycle Kit (schema-backed public receipts and CLI JSON envelopes — публичные доказательства и JSON-конверты CLI со схемами)
