...
> **Статус заполнения артефактов:** все 37 / 37 исследовательских артефактов, включая bb #36 и T3 Code #37, подготовлены, включены в сводку и расчёты и приняты. TASK bb завершена после процессной приёмки и зелёного CI. T3 Code перенесён из статейного трека 2026-09-11.
...
### 3. Разделение на семнадцать уровней абстракции

**Все 36 подготовленных исследовательских артефактов** чётко делятся на семнадцать уровней:
...
| **Agentic IDE / Programmable control surface** (агентная среда разработки и программируемая управляющая поверхность) | bb | Нормализует внешних поставщиков в threads (потоки), предоставляет UI/CLI/API/SDK, managed worktrees (управляемые рабочие деревья) и опциональный durable Workflows plugin (устойчивый плагин процессов) | Потенциальная ручная поверхность и источник provider/thread/replay patterns (паттернов поставщиков, потоков и воспроизведения); не core dependency |
...
**28 из 36 подготовленных исследовательских артефактов** соответствуют этому критерию. Ровно учтённые проекты:
1. Crush (#1)
2. pi_agent_rust (#2)
3. CrewAI (#3)
4. OpenHands SDK (#6)
...
**20 из 36 подготовленных исследовательских артефактов** подтверждают поддержку MCP:
...
- bb не увеличивает числитель: ручной `/compact` и provider-native memory/compaction зависят от внешнего поставщика; собственного универсального auto-compaction ядра не подтверждено
...
|---|---|---|
| **Python** | CrewAI, LangGraph, AutoGen, OpenHands SDK, MetaGPT, Agno, Hermes Agent, **bx-dev** (support scripts; primary artifact is Markdown skill), **Agent Lifecycle Kit** | Доминирующий язык для AI-agent фреймворков и support tooling |
| **TypeScript** | Archon, OpenClaw, Mastra AI, Paperclip AI, Sandcastle, Kilo Code, **OpenCode**, **Multica** (frontend), Orca ADE, **bb** | Растущая экосистема, особенно для workflow engines, мета-оркестраторов, ADE и sandbox orchestration |
