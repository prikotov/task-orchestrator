...
Crush **не является** фреймворком оркестрации агентов. Это **CLI-инструмент** для интерактивной работы разработчика с LLM с поддержкой субагентов (Coder → Task). В отличие от task-orchestrator, Crush не поддерживает цепочки шагов, retry-механизмы с backoff, circuit breaker, budget control или quality gates.
...
| **Sub-agent / multi-agent** | ❌ Нет | ✅ Coder → Task sub-agent (иерархический, через tool `agent`) | 🟡 Позже |
...
| **Loop detection** | ❌ Нет | ✅ SHA-256 сигнатуры, скользящее окно (10/5) | 🟡 Интересно |
...
### 3.2 🟡 Loop Detection — обнаружение повторяющихся tool calls (`internal/agent/loop_detection.go`)
...
### 3.7 🟡 Sub-Agent архитектура — Coder → Task (`internal/agent/`)

**Что у них:** Crush реализует иерархическую мультиагентную архитектуру:

1. **Coder agent** — основной агент с полным набором инструментов (edit, bash, write, view, ...)
2. **Task agent** — субагент с read-only инструментами (glob, grep, ls, sourcegraph, view)

Coder может вызвать tool `agent`, который создаёт дочернюю сессию и запускает Task agent с отдельным промптом. Cost субагента аккумулируется на родительской сессии.

```go
// coordinator.go: Coder agent получает tool "agent" если он в AllowedTools
...
Task agent имеет read-only инструменты (`resolveReadOnlyTools`) и не имеет доступа к MCP/LSP по умолчанию.

**Почему нам интересно:** Иерархия «основной агент → субагент для поиска контекста» — это паттерн, который task-orchestrator мог бы использовать для выделения фаз поиска/анализа в отдельные шаги.
...
| Sub-agent (Coder → Task) | 🟡 P3 | Иерархия «основной агент → субагент поиска» |
...
- [`internal/agent/coordinator.go`](https://github.com/charmbracelet/crush/blob/main/internal/agent/coordinator.go) — Coordinator: управление моделями, провайдерами, инструментами, sub-agent'ами (Coder → Task)
...
- [`internal/agent/loop_detection.go`](https://github.com/charmbracelet/crush/blob/main/internal/agent/loop_detection.go) — Loop detection: SHA-256 сигнатуры tool calls, скользящее окно
...
- [`internal/config/config.go`](https://github.com/charmbracelet/crush/blob/main/internal/config/config.go) — Конфигурация: providers, models, agents (Coder/Task), context paths
