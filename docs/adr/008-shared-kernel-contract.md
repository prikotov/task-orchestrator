...
- Roadmap-сценарии (conditional branching, параллельное выполнение) добавляют новые типы chain-specific данных.
...
**Общее ядро (Shared Kernel) = chain identity (name, budget, roles). Strategy-specific данные НЕ входят.**
...
Новые стратегии добавляют данные через sub-интерфейсы, не модифицируя общее ядро:
...
P4 (расщепление `ChainDefinitionVo` и формализация общего ядра) реализуется **до** ExecutionStrategy (задачи #7–9 из плана действий brainstorm-сессии). Это обеспечивает:
...
- **OCP-совместимое расширение:** conditional branching, параллельное выполнение, sub-agents добавляются через sub-интерфейсы без модификации `ChainDefinitionInterface`.
...
- **Security Policy как cross-cutting concern:** будущий модуль Security Policy может потребовать доступа к данным обеих стратегий. Общее ядро при этом разрастается. Это открытый вопрос (владелец: Локи, задача #14 из плана действий).

## Альтернативы

1. **Общее ядро = все данные ChainDefinitionVo:** формальный контракт включает все 20 параметров. Отвергнуто — defeats the purpose: strategy-specific данные (шаги, retry-политика) не являются общими между context'ами и будут загрязнять контракт.

2. **ISP-рефакторинг (Interface Segregation) без общего ядра:** расщепить ChainDefinitionVo на интерфейсы по ISP, но не выделять общий root. Отвергнуто — при добавлении новой стратегии нет гарантии, что «общие» методы действительно общие; формальный root-интерфейс обеспечивает дисциплину.
