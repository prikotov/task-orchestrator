...
- [Идентичность агента (Agent Identity)](agent-identity.md) — разделение идентичности AI-агента и владельца репо через GitHub App (имя выбирает пользователь), команда `bin/console agent:token` (модуль GitIdentity)
