...
**Решение для локального режима сопровождения этого репозитория:** не применяйте force-push или `--admin`. Остановитесь и согласуйте восстановление с пользователем. Новые ветки с первого push отправляйте через [`bin/push-pr-branch`](agent-identity.md#локальный-режим-проекта-push-pr-веток-токеном-бота) — безопасный helper публикации PR-ветки токеном настроенного GitHub App.
