...
| Runner | Поддержка | Механизм |
|---|---|---|
| **pi** (>= v0.67.4) | ✅ Да | CLI-флаг `-nc` / `-no-context-files` |
| **Codex CLI** | ⏳ Не внедрён | TBD при реализации `CodexAgentRunner` |
| **Gemini CLI** | ⏳ Не внедрён | TBD при реализации `GeminiAgentRunner` |
| **OpenCode** | ⏳ Не внедрён | TBD при реализации `OpenCodeAgentRunner` |
| **Kilocode** | ⏳ Не внедрён | TBD при реализации `KilocodeAgentRunner` |

При добавлении нового runner'а (`*AgentRunner implements AgentRunnerInterface`):
1. Если CLI поддерживает отключение контекста — передать нужный флаг при `getNoContextFiles() === true`
2. Если не поддерживает — проигнорировать флаг (контекст будет загружен всегда)
3. При необходимости — логировать debug-предупреждение о неподдерживаемой опции
