...
## 2. Решение: GitHub App (не бот-аккаунт)
...
- **Большой blast radius (радиус поражения).** PAT долгоживущий и привязан к аккаунту, а не к репо. Утечка = доступ ко всем репо, где аккаунт — collaborator (соавтор).
...
Имя App — **не захардкожено** в product и не предписывается этим гайдом как конкретная строка. Это **свойство GitHub**: slug (короткое имя) App глобально уникален на всей платформе, поэтому каждый пользователь выбирает свободное имя самостоятельно.
...
**Почему PEM НЕ в `var/`:** PEM — это не временный артефакт, а долговременное средство подписи JWT. Его перевыпуск возможен только вручную через GitHub UI (см. [Ротация PEM](#8-ротация-pem-private-key)). Положить PEM в `var/` — значит рисковать потерять доступ при любой чистке кеша. Поэтому постоянные секреты живут в `secrets/`, а кеш короткоживущих токенов — в `var/`.
