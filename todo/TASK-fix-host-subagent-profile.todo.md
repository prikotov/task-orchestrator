...
- [ ] Установленный Composer пакет читает `config/chains.yaml` потребляющего проекта.
- [ ] Для разбора YAML используется автозагрузчик потребляющего проекта.
- [ ] Запуск из checkout пакета сохраняет работоспособность.
- [ ] Есть регрессионный интеграционный тест с fake runner (фиктивный движок).

### 🟡 Желательно (Should Have)
- [ ] Проверка воспроизводится в `make check`.
