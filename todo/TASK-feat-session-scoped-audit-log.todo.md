...
*   **Где делаем:**
    *   `src/Common/Module/Orchestrator/Infrastructure/Service/Chain/ChainSessionLogger.php` — создание audit.jsonl внутри сессии
    *   `src/Common/Module/Orchestrator/Infrastructure/Service/Chain/JsonlAuditLogger.php` — без изменений (фабрика создаёт с нужным путём)
    *   `src/Common/Module/Orchestrator/Application/UseCase/Command/OrchestrateChain/OrchestrateChainCommandHandler.php` — передавать путь `<sessionDir>/audit.jsonl` вместо глобального
