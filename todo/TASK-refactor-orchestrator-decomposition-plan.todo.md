...
- [ ] ADR-006: ExecutionStrategy composition — зафиксировать решение, альтернативы (наследование, tagged union), критерий реализации (conditional branching)
- [ ] ADR-007: VO ACL между Orchestrator и AgentRunner — зафиксировать ACL как осознанное решение, порог пересмотра (>3 общих поля или typed I/O)
- [ ] ADR-008: Shared Kernel Contract — Shared Kernel = chain identity (name, budget, roles). Strategy-specific data НЕ входит
- [ ] Задача TASK-refactor-inline-execute-dynamic-turn — инлайнинг ExecuteDynamicTurnService (P1 action item #1)
- [ ] Задача TASK-refactor-prompt-configuration-vo — PromptConfiguration VO (P1 action item #2)
- [ ] Задача TASK-refactor-session-writer-consumers — переключение 3 потребителей на ChainSessionWriterInterface (P1 action item #3)

### 🟡 Should Have (Желательно)
- [ ] Черновой roadmap в `docs/releases/` на 2 квартала с привязкой conditional branching / parallel execution к спринтам
...
1. [ ] Делегировать Архитектору Гэндальфу: создание ADR-006, ADR-007, ADR-008
2. [ ] Делегировать Аналитику Шерлоку: черновой roadmap в `docs/releases/`
3. [ ] Тимлид: создать задачи на P1-action items (#1–3) в `todo/`
4. [ ] Обновить эпик: добавить Фазу 2 как выполненную, ссылки на новые задачи
5. [ ] Самопроверка: все ADR соответствуют формату, задачи соответствуют шаблону

## 5. Definition of Done (Критерии приёмки)
- [ ] 3 ADR созданы в `docs/adr/` (ADR-006, ADR-007, ADR-008)
- [ ] ≥ 3 задачи на P1-action items созданы в `todo/`
- [ ] Эпик обновлён (Фаза 2 отмечена, ссылки актуальны)
- [ ] Все файлы задач содержат обязательные метаданные и соответствуют шаблону
