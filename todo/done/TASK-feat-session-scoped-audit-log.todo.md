...
- [x] Audit log создаётся внутри директории сессии: `var/sessions/<chain>/<timestamp>/audit.jsonl`
- [x] Глобальный `var/log/audit.jsonl` больше не используется
- [x] `audit_log_path` убран из DI-конфигурации (Configuration.php, TaskOrchestratorExtension.php)
- [x] Session resume дозаписывает в существующий audit.jsonl
- [x] PHPUnit: существующие тесты проходят
...
1. [x] В `ChainSessionLogger::startSession()` — создать `audit.jsonl` путь, вернуть через VO или getter
2. [x] В `OrchestrateChainCommandHandler` — получить audit path из session logger, передать в фабрику
3. [x] Убрать `audit_log_path` из Configuration.php, TaskOrchestratorExtension.php, bin/console
4. [x] Обновить console_services.yaml — убрать дефолтный AuditLoggerInterface
5. [x] Протестировать: `php bin/console app:agent:orchestrate "test" -c brainstorm`
6. [x] Проверить: audit.jsonl лежит рядом с session.json

## 5. Definition of Done (Критерии приёмки)
- [x] После brainstorm в `var/sessions/<chain>/<timestamp>/` есть `audit.jsonl` с записями
- [x] Глобальный `var/log/audit.jsonl` не создаётся
- [x] `--no-audit-log` флаг работает
...
| 2026-04-20 | Тимлид (Алекс) | Закрытие задачи: PR #30 merged, статус → done |
