...
- Переносимый набор навыков постановки: `task-definition` (одиночная задача), `epic-definition` (концепция эпика), `epic-decomposition` (декомпозиция эпика) с общим протоколом подготовки постановок.
...
- [x] Навык `epic-definition`: подготовка концепции эпика (проблема, цель, границы, требования); декомпозиция — отсылкой к `epic-decomposition`, если входила в запрос.
- [x] Навык `epic-decomposition`: декомпозиция принятой концепции эпика на задачи (требование → задача → проверка) с зависимостями без циклов.
