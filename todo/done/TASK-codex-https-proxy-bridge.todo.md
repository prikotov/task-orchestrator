...
Upstream-прокси (например, `prikotov.pro:33090`) принимает только TLS-соединения.
...
- Python-прототип моста (30 строк), проверенный в E2E: работает с codex через `prikotov.pro:33090`
...
Проблема обнаружена при E2E-тестировании: прокси `prikotov.pro:33090` принимает только TLS-соединения. Codex CLI (reqwest 0.12.28) поддерживает только `http://` схему для прокси. Python-прототип моста (HTTP → TLS upstream) подтвердил работоспособность — codex через мост вернул `PING_OK` от OpenAI API.
