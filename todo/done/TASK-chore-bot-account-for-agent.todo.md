...
| 2026-06-22 | Тимлид Алекс | Финальная реализация отличается от исходной постановки: standalone `bin/agent-token` заменён на полноценный DDD-модуль `src/Module/GitIdentity` + Symfony-команду `bin/console agent:token` (по конвенциям). Параллельно — переход на Symfony Kernel + ModuleSystem (library→application). Описание задачи выше описывает промежуточный артефакт; актуальная реализация — в коде и гайде `docs/guide/agent-identity.md`. Проверки финал: 1220 тестов, psalm 0, deptrac 0, phar-smoke pass. |
