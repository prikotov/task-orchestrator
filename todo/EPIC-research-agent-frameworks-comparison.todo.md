...
- [ ] [TASK-research-pragmatic-orchestration](TASK-research-pragmatic-orchestration.todo.md) — pragmatic-orchestration (`CodeAlive-AI/pragmatic-orchestration`, Python 3.11+ + Bash, MIT, создан 2026-09-17, snapshot main 2026-09-21; orchestration-слой над внешними CLI-агентами в форме Agent Skills + `porch` CLI: steerable delegation (стерируемая делегация — перенаправление работающего агента: файловый mailbox + supervisor + адаптеры харнесов, режимы auto/queue/interrupt, evidence-ранги доставки), мульти-модельное ревью с LLM-судьёй (basic/specialists/super/ultra), закрытая схема событий PorchEvent, fail-closed матрица режимов доступа, sessions-поиск по историям харнесов, quota; поддержка Codex CLI, Claude Code, OpenCode, Grok Build, Devin CLI, Gemini — review-only; скилл remote-agents — выделенные хосты SSH-over-SSM/WireGuard/Terraform). Классификация: `orchestration skill suite / delegation layer over external agents` (не агент, не chain engine), ближайшие аналоги qm (#32) / omnigent (#33) / bx-dev (#31) / Herdr (#34). Предварительный verdict: 🟡 паттерны (steer-механика, закрытая схема событий, deviation journal, fail-closed матрица) / 🔴 не dependency (Python+Bash skill, не встраивается в PHP-движок).
