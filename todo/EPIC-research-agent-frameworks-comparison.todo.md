...
status: in_progress
reopened: 2026-09-11
pr: "#51 (исследование), #52 (ревью и исправления), #97 (Paperclip AI + AgentCraft, финализация), #340 (Herdr; замена #339), #385 (T3 Code, перенос из статейного трека)"
---
...
Трек ведётся в evergreen-режиме (вечнозелёный): появление нового агент-оркестратора → новая задача Этапа 1 + строка в сводной таблице. Фиксированный дедлайн Goal («до конца Q2 2026») не применяется, текст Goal сохраняется как исторический.
...
### Этап 1p: Дополнительное исследование (2026-09-22)

- [x] [TASK-research-pragmatic-orchestration](done/TASK-research-pragmatic-orchestration.todo.md) — pragmatic-orchestration (`CodeAlive-AI/pragmatic-orchestration`, Python 3.11+ + Bash, MIT, создан 2026-09-17, snapshot main 2026-09-21; orchestration-слой над внешними CLI-агентами в форме Agent Skills + `porch` CLI: steerable delegation (стерируемая делегация — перенаправление работающего агента: файловый mailbox + supervisor + адаптеры харнесов, режимы auto/queue/interrupt, evidence-ранги доставки), мульти-модельное ревью с LLM-судьёй (basic/specialists/super/ultra), закрытая схема событий PorchEvent, fail-closed матрица режимов доступа, sessions-поиск по историям харнесов, quota; поддержка Codex CLI, Claude Code, OpenCode, Grok Build, Devin CLI, Gemini — review-only; скилл remote-agents — выделенные хосты SSH-over-SSM/WireGuard/Terraform). Классификация: `orchestration skill suite / delegation layer over external agents` (не агент, не chain engine), ближайшие аналоги qm (#32) / omnigent (#33) / bx-dev (#31) / Herdr (#34). Предварительный verdict: 🟡 паттерны (steer-механика, закрытая схема событий, deviation journal, fail-closed матрица) / 🔴 не dependency (Python+Bash skill, не встраивается в PHP-движок).

### Этап 1q: Дополнительное исследование (2026-09-23)

- [x] [TASK-research-strands-harness-sdk](done/TASK-research-strands-harness-sdk.todo.md) — Strands Harness SDK (`strands-agents/harness-sdk`, snapshot `fec042766488cfb2627f1998897b1cc0fc238d3f`; Python 3.10+ и TypeScript, Apache-2.0; `SDK / agent framework with assembled harness`: встроенный цикл `model → tool → model`, собранный `create_harness()`, lifecycle controls, hooks/interventions, sessions/memory/context, `Graph`/`Swarm`/A2A, Evals SDK и OpenTelemetry). Исследовательские артефакты: `strands-harness-sdk-comparison.md`, строка #39, сводка 39/39. Ревью Бэка Пуаро: одобрено (20 сверок, 19 подтверждены, 2 минорных CR устранены). Verdict: 🟡 паттерны (termination/policy/defaults/state/observability/eval) / 🔴 не dependency.
...
- [ ] Evergreen-режим (решение владельца 2026-09-11): эпик закрывается ТОЛЬКО явным решением владельца, а не автоматически по исчерпанию задач. Пока эпик открыт — сводная таблица `docs/research/agent-frameworks-summary.md` остаётся актуальной: comparison-артефакт и строка на каждый исследованный продукт.
...
| 2026-08-29 06:34:56 (1787960096) | Аналитик (Шерлок) | Эпик reopened стадией `1o`: создана постановка TASK-research-bb-agent-ide по `getbb.app`. bb предварительно классифицирован как `agentic IDE / control surface` над внешними coding agents, а не самостоятельный coding agent; ближайшие аналоги — Orca ADE, qm, omnigent и Herdr. Исследование и research-артефакты на этапе постановки не выполнялись. |
...
| 2026-09-11 | Тимлид (Алекс) | По решению владельца за эпиком закреплён evergreen-режим: новые агент-оркестраторы пополняют трек новыми задачами Этапа 1; закрытие — только явным решением владельца. |
| 2026-09-22 | Тимлид (Алекс) | Эпик открыт стадией `1p`: создана постановка TASK-research-pragmatic-orchestration по итогам предварительного ресерча `CodeAlive-AI/pragmatic-orchestration` (предложение пользователя подтверждено). Классификация предварительно: orchestration skill suite / delegation layer над внешними CLI-агентами; исследование и research-артефакты на этапе постановки не выполнялись. |
| 2026-09-22 | Аналитик (Шерлок) | Stage `1p`: по snapshot main 2026-09-21 (commit `a72695dd…`) подготовлены `pragmatic-orchestration-comparison.md`, строка #38 в summary (сводка 38/38, синхронизированы также тренды за T3 Code #37) и отчёт агента; 7 гипотез тимлида проверены (5 подтверждены, 2 с уточнениями). Ревью Бэка Пуаро: одобрено, 12/12 выборочных сверок с первоисточником, 4 минорных замечания устранены. |
| 2026-09-22 | Тимлид (Алекс) | Stage `1p`: после зелёного CI задача переведена в `done`; итоговый verdict: 🟡 паттерны (steer delivery lifecycle, closed events, fail-closed матрица, deviation journal, fake-CLI tests) / 🔴 не dependency. PR #406 готов к апруву. |
| 2026-09-22 | Аналитик (Шерлок) | Stage `1p`: по snapshot `a72695dded7e2f28c185c3a2d770b26eb45528fa` подготовлены `pragmatic-orchestration-comparison.md`, отчёт агента и строка #38; сводка доведена до 38/38. Подтверждены паттерны steer mailbox/evidence, closed events, fail-closed capabilities, deviation journal, native session locators и fake-CLI tests; уточнено, что semantic dedup review выполняет LLM-судья. Verdict: 🟡 паттерны / 🔴 не dependency; ACP не рекомендован для текущего batch-контракта. TASK и чекбокс стадии оставлены `in_progress` до процессной приёмки. |
| 2026-09-23 | Аналитик (Шерлок) | Stage `1q`: по snapshot `fec042766488cfb2627f1998897b1cc0fc238d3f` подготовлены `strands-harness-sdk-comparison.md` и строка #39; сводка доведена до 39/39. Проверены семь гипотез: три подтверждены, две уточнены, одна подтверждена с отрицательным текущим решением, одна опровергнута терминологически при подтверждённой потребности. Verdict: 🟡 заимствовать termination/policy/defaults/state/observability/eval patterns / 🔴 не dependency. TASK и чекбокс стадии оставлены `in_progress` до процессной приёмки. |
| 2026-09-23 | Тимлид (Алекс) | Stage `1q`: самопроверка (2 замечания — терминология doc-style и неоднозначность ограничителей A2A) и ревью Бэка Пуаро (20 выборочных сверок, 19 подтверждены) пройдены, 2 минорных CR ревью устранены. Задача переведена в `done`, PR #407 создан от имени `prikotov-agent[bot]` и готов к апруву. |
