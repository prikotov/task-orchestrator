...
- [ ] [TASK-research-strands-harness-sdk](TASK-research-strands-harness-sdk.todo.md) — Strands Harness SDK (`strands-agents/harness-sdk`, snapshot `fec042766488cfb2627f1998897b1cc0fc238d3f`; Python 3.10+ и TypeScript, Apache-2.0; `SDK / agent framework with assembled harness`: встроенный цикл `model → tool → model`, собранный `create_harness()`, lifecycle controls, hooks/interventions, sessions/memory/context, `Graph`/`Swarm`/A2A, Evals SDK и OpenTelemetry). Исследовательские артефакты подготовлены: `strands-harness-sdk-comparison.md`, строка #39 и сводка 39/39; задача остаётся `in_progress` до процессной приёмки. Verdict: 🟡 паттерны / 🔴 не dependency.
