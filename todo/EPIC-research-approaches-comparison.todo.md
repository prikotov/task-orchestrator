...
Role-based процесс разработки с AI-агентами (роли команды, скиллы, конвенции, AGENTS.md) складывался эмпирически: внешние подходы и процессы (SDLC/PDLC, spec-driven development, dark vs lit factory, context engineering, механизмы качества) не исследованы систематически. Процесс развивается вслепую — есть риск упустить проверенные практики и повторять чужие ошибки.

### Варианты или путь решения (Solution Sketch)
Серия исследовательских задач по единой методологии из 8 критериев: каждый подход изучается отдельной задачей, по нему пишется comparison-отчёт с маппингом на наш процесс и вердиктом adopt / adapt / reject. Финальная задача сводит результаты в таблицу с классификацией и рекомендациями.

### Ожидаемый результат (Expected Result)
Процесс task-orchestrator подкреплён исследовательской базой: по каждому внешнему подходу есть обоснованный вердикт (adopt / adapt / reject) с оценкой усилий и маппингом на роли/скиллы/конвенции. Решения об изменении процесса принимаются осознанно, а не по наитию.
...
Трек ведётся в evergreen-режиме (вечнозелёный): появление нового подхода/процесса оркестрации AI-агентов → новая задача Этапа 1 + строка в сводной таблице. Фиксированный дедлайн Goal не применяется, текст Goal сохраняется как исторический. Закрытие — только явным решением владельца.
...
- [ ] Evergreen-режим (решение владельца 2026-09-13): эпик закрывается ТОЛЬКО явным решением владельца, а не автоматически по исчерпанию задач. Пока эпик открыт — сводная таблица `docs/research/approaches-summary.md` остаётся актуальной: comparison-артефакт и строка на каждый исследованный подход.
...
## 9. Sources (Источники)
- Прецеденты: `todo/EPIC-research-coding-agents-comparison.todo.md`, `todo/done/EPIC-research-agent-frameworks-comparison.md`
...
| 2026-08-16 | Бэкендер Левша (pi) | Миграция на формат todo-md 0.0.10: файл переименован в `EPIC-*.todo.md`, добавлены разделы Human Brief и поле `pr`, формат author/assignee приведён к виду «Роль (агент)». |
| 2026-09-13 | Тимлид Алекс (pi) | По решению владельца за эпиком закреплён evergreen-режим: новые подходы/процессы оркестрации пополняют трек задачами Этапа 1; закрытие — только явным решением владельца, а не автоматически по исчерпанию задач. |
